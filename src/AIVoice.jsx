...
export default function AIVoice({ apiBase, token, brandId, onSent }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/voice/styles", { headers: H });
  ...
  async function loadClips() {
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/voice?brand=${brandId}`, { headers: H });
      setClips(await r.json());
    } catch (_) {}
  }
  async function loadPending() {
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/content?brand=${brandId}&status=pending`, { headers: H });
    ...
    try {
      ...
      mr.onstop = async () => {
        ...
        try {
          ...
          const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/voice/transcribe", {
  ...
  async function makeScript() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/voice/script", {
  ...
  async function makeVoice() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/voice/generate", {
        method: "POST", headers: H,
  ...
  async function makeTwoVoices() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/variants/voice", {
        method: "POST", headers: H,
  ...
  async function attach(voiceId, contentId) {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/voice/attach", {
  ...
  async function dlSubs() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/voice/subtitles", {
        method: "POST", headers: H, body: JSON.stringify({ subtitles: subs }),
      });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error);
      window.open(apiBase.replace(/\/+$/, "") + j.url, "_blank");
    } catch (e) { setErr(e.message); }
  }
        ...
        <div className="space-y-3">
          {clips.map((c) => (
            <div key={c._id} className="rounded-xl bg-neutral-800/60 border border-neutral-700 p-3">
              ...
              <audio controls preload="none" src={apiBase.replace(/\/+$/, "") + c.file} className="w-full h-9" />

              <div className="flex gap-2 mt-2">
                <a href={apiBase.replace(/\/+$/, "") + c.file} download className="flex-1 text-center text-xs py-2 rounded-lg border border-neutral-700 text-neutral-300">⬇ Download</a>
