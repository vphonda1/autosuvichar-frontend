...
export default function LuckyDrawEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  // ── बोलकर बनवाया हुआ draft — असली गाड़ियाँ ─────────────────
  useEffect(() => {
    if (!draft) return;
    if (Array.isArray(draft.bikes) && draft.bikes.length) {
      setBikes((prev) => prev.map((b, i) => draft.bikes[i]
        ? { ...b, name: draft.bikes[i].name || b.name, img: draft.bikes[i].img || b.img }
        : b));
    }
    if (draft.subTitle) {
      setCaption(
        `🪔 ${draft.shopName || ""} के साथ इस त्यौहार में जीतें Mega Lucky Draw! ${draft.subTitle}\n` +
  ...
  const render = useCallback(() => {
    ...
    bikes.forEach(bk => {
      if (bk.img) {
        ...
        if (im) drawFit(ctx, im, bk.x, bk.y, bk.w, bk.h);
      } else {
  ...
  }, [bg, bikes, gifts, elems, selId, brandId, apiBase, logoTick, ownerTick, imgCache.tick]);
