...
export default function BrandMemory({ apiBase, token, brandId }) {
  ...
  async function load() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/brand-profile/${scope}`, { headers: { Authorization: "Bearer " + token } });
  ...
  async function save() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/brand-profile/${scope}`, {
        method: "PUT",
  ...
  async function addFeedback(liked) {
    ...
    vib(40); setBusy(true);
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/brand-profile/${scope}/feedback`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
  ...
  async function removeNote(liked, idx) {
    ...
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/brand-profile/${scope}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
