...
export default function ThreeChoice({ apiBase, token, brandId, accent = "#E4002B", onDone, forcedType, hideOwnInput, sharedText, autoTrigger }) {
  ...
  // ⚠️ पहले सिर्फ़ अपने भीतरी "txt" state पर निर्भर था — अब बाहर के
  //    एक-जगह-वाले panel से भी text/प्रकार सीधे भेजे जा सकते हैं
  async function banao(overrideText, overrideType) {
    ...
    try {
      ...
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/three/make`, {
      ...
      pollRef.current = setInterval(async () => {
        n++;
        try {
          const rr = await fetch(`${apiBase.replace(/\/+$/, "")}/api/three/${d.id}`, { headers: auth });
  ...
  // बाहर के panel से "बनाओ" दबाने पर
  ...
  async function chuno(tone) {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/three/${job.id}/pick`, {
  ...
  async function sabHatao() {
    ...
    vib([20, 30, 20]);
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/three/${job.id}`, { method: "DELETE", headers: auth });
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── लिखिए — बाहर से control हो तो छिप जाए ────────────── */}
      {!hideOwnInput && (
