...
export default function AINews({ apiBase, token, brandId, onSent }) {
  ...
  async function load() {
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/news?brand=${brandId}${onlyVerified ? "&verified=1" : ""}`, { headers: H });
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/news/sources", { headers: H });
  ...
  async function fetchNews() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/news/fetch", {
  ...
  async function verify(id, val) {
    setErr(""); vib(25);
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/news/${id}/verify`, {
  ...
  async function toPost(id) {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/news/${id}/to-post`, {
        method: "POST", headers: H, body: JSON.stringify({ brand: brandId }),
      });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Error");
  ...
  async function del(id) {
    vib(20);
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/news/${id}`, { method: "DELETE", headers: H });
      await load();
    } catch (_) {}
  }
