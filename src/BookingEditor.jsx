...
export default function BookingEditor({ apiBase, token, brandId, onSent }) {
  ...
  const render = useCallback(() => {
    ...
    ctx.beginPath(); ctx.roundRect(stx, secY, stw, 60, 30); ctx.fill();
    ctx.textAlign = "left";
    ...
    if (bikeImg) {
      const img = new Image(); img.src = bikeImg;
      if (img.complete) {
        ctx.drawImage(img, W*0.52, H*0.44, W*0.46, H*0.36);
      } else { img.onload = () => render(); }
    } else {
      ctx.save();
      ctx.globalAlpha = 0.1;
      ctx.fillStyle = "#E4002B";
      ctx.beginPath(); ctx.roundRect(W*0.54, H*0.46, W*0.42, H*0.32, 20); ctx.fill();
      ctx.restore();
      ctx.textAlign = "center";
