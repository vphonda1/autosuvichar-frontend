...
export default function BookingEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  useEffect(() => {
    if (!draft) return;
    if (draft.mainTitle) setMainTitle(draft.mainTitle);
    if (draft.subTitle) setSubTitle(draft.subTitle);
    if (draft.bikeImg) setBikeImg(draft.bikeImg);
    if (draft.shopName) { setDealerName(draft.shopName); setTouched(true); }
    if (draft.shopSub) setDealerSub(draft.shopSub);
    if (draft.phone) setPhone(draft.phone);
    if (draft.vehicleName) {
      setCaption(
        `🪔 ${draft.vehicleName} की बुकिंग शुरू! अभी बुक करें और पाएं धमाकेदार फायदे।\n` +
        ...
        <div className="mt-2">
          <VehiclePicker apiBase={apiBase} token={token} brandId={brandId}
            onPick={(v) => {
              ...
              if (v.fullName) setMainTitle(v.fullName);
              const line = [v.dpLine, v.emiLine, v.offerLine].filter(Boolean).join(" · ");
              if (line) setSubTitle(line);
            }} />
        </div>
