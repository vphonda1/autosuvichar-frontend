...
export default function DeliveryEditor({ apiBase, token, brandId, onSent }) {
  ...
  function confirmCrop() {
    ...
    finalImg.onload = () => {
      const s = Math.min(720 / finalImg.naturalWidth, 760 / finalImg.naturalHeight, 1.6);
      setPhotoDim({ w: Math.round(finalImg.naturalWidth * s), h: Math.round(finalImg.naturalHeight * s) });
      setPhoto(cropped);
    };
    finalImg.src = cropped;
    setCropSrc(null);
  }
  ...
  const inner = `
    ...
    ${addr3D()}
    <g data-el="logo" transform="translate(${pos.logo.x},${pos.logo.y})" style="cursor:move"><image href="/logos/vp-honda.png" x="0" y="0" width="124" height="124"/></g>
