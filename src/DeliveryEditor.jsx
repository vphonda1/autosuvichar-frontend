...
export default function DeliveryEditor({ apiBase, token, brandId, onSent }) {
  ...
  const inner = `
    ...
    <rect x="0" y="${H * 0.91}" width="${W}" height="${H * 0.09}" fill="#141414"/><rect x="0" y="${H * 0.91}" width="${W}" height="5" fill="${GOLD}"/>
    <text x="540" y="${H * 0.957}" text-anchor="middle" font-family="Noto Sans Devanagari,Arial,sans-serif" font-size="36" font-weight="800" fill="#fff">${esc(f.brand)}  •  फ़ोन ${esc(f.phone)}</text>
    <text x="540" y="${H * 0.99}" text-anchor="middle" font-family="Noto Sans Devanagari,Arial,sans-serif" font-size="24" fill="#ddd">${esc(f.place)}</text>
