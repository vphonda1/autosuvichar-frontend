...
export default function AICommandCenter({ apiBase, token, brandId, onSent }) {
  ...
  async function understand() {
    ...
    try {
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/command/understand", {
        method: "POST",
  ...
  async function execute() {
    ...
    try {
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/command/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
        body: JSON.stringify({
  ...
  async function makePoster() {
    ...
    try {
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/command/poster-spec", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
        body: JSON.stringify({
  ...
  async function loadScheduled() {
    try {
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/command/scheduled?brand=" + brandId, {
  ...
  async function cancelScheduled(id) {
    vib(30);
    try {
      await fetch(apiBase.replace(/\/+$/, "") + "/api/command/scheduled/" + id, { method: "DELETE", headers: { Authorization: "Bearer " + token } });
      loadScheduled();
    } catch (_) {}
  }
