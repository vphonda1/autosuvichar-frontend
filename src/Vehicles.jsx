...
// ⚠️ server कभी-कभी JSON की जगह HTML भेज देता है (file बहुत बड़ी हो, या
//    service जाग रही हो)। तब "Unexpected token '<'" जैसा गूढ़ सन्देश आता था।
//    अब साफ़ हिन्दी में बात कही जाती है।
async function jsonSafe(r) {
  ...
  try { return JSON.parse(t); }
    ...
    if (/^\s*<!DOCTYPE|^\s*<html/i.test(t)) {
      throw new Error(
        r.status === 413 ? "photo बहुत बड़ी है — छोटी करके डालिए"
        : r.status === 502 || r.status === 503 ? "server अभी जाग रहा है — एक मिनट बाद कोशिश कीजिए"
        : `server ने जवाब नहीं दिया (${r.status})`
      );
    }
    throw new Error(t.slice(0, 120) || `गड़बड़ (${r.status})`);
  }
}
...
export default function Vehicles({ apiBase, token, brandId }) {
  ...
  const full = (u) => (u ? (u.startsWith("http") ? u : apiBase.replace(/\/+$/, "") + u) : "");

  async function load() {
    setLoading(true);
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/vehicles?brand=${scope}`, { headers: auth });
  ...
  async function uploadPhoto(id, file) {
    ...
    try {
      ...
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/vehicles/${id}/photo`, { method: "POST", headers: auth, body: fd });
      const d = await jsonSafe(r);
  ...
  async function removePhoto() {
    ...
    vib([20, 30, 20]);
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/vehicles/${editId}/photo`, { method: "DELETE", headers: auth });
  ...
  async function save() {
    ...
    try {
      ...
      const url = editId ? `${apiBase.replace(/\/+$/, "")}/api/vehicles/${editId}` : `${apiBase.replace(/\/+$/, "")}/api/vehicles`;
  ...
  async function del(id) {
    vib([20, 30, 20]);
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/vehicles/${id}`, { method: "DELETE", headers: auth });
  ...
  async function toggleStock(v) {
    vib(20);
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/vehicles/${v._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", ...auth },
  ...
  //  ⚠️ नया — "सभी गाड़ियाँ एक साथ" वाली एक PNG। त्यौहार/बधाई वाले poster
  //     में अलग-अलग गाड़ियों की बेमेल कतार की जगह यही एक साफ़ तस्वीर लगती
  //     है। background-रहित (transparent) PNG सबसे अच्छी दिखती है।
  //  ⚠️ बड़ी गड़बड़ी थी — तस्वीर हमेशा ऊपर चुने हुए brand में सेव होती थी,
  //     चाहे आप नीचे किसी और brand की सूची देख रहे हों। इसीलिए Yakuza की
  //     तस्वीर VP Honda में चली गई और दोनों के poster गड़बड़ हो गए।
  const fleetBrand = (scope && scope !== "all") ? scope : brandId;
  const [fleetHas, setFleetHas] = useState(false);
  const [fleetBusy, setFleetBusy] = useState("");
  useEffect(() => {
    fetch(`${apiBase.replace(/\/+$/, "")}/api/brand-fleet-photo?brand=${fleetBrand}`)
      .then((r) => r.json()).then((d) => setFleetHas(!!d.has)).catch(() => {});
  }, [fleetBrand, apiBase]);
  async function uploadFleet(file) {
    if (!file) return;
    setFleetBusy("भेज रहे हैं…");
    try {
      const fd = new FormData();
      fd.append("photo", file);
      fd.append("brand", fleetBrand);   // ⚠️ जिस brand की सूची खुली है, उसी में सेव हो
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/brand-fleet-photo`, {
        method: "POST", headers: { Authorization: "Bearer " + token }, body: fd,
      });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "नहीं चढ़ी");
      setFleetHas(true); setFleetBusy(`✅ चढ़ गई (${d.kb}KB)`);
      setTimeout(() => setFleetBusy(""), 3000);
    } catch (e) { setFleetBusy("❌ " + e.message); }
  }

  return (
    <div className="space-y-3 pb-10">

      {/* सभी गाड़ियाँ एक साथ — त्यौहार वाले poster के लिए */}
      <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-3 space-y-2">
        <p className="text-xs font-bold text-yellow-400">🏍️ सभी गाड़ियाँ एक साथ वाली तस्वीर — {BRAND_LABELS[fleetBrand] || fleetBrand}</p>
        <p className="text-[10px] text-neutral-500 leading-relaxed">
          त्यौहार/बधाई वाले poster में यही तस्वीर लगेगी — एक ही PNG में सारी गाड़ियाँ।
          background-रहित (transparent) PNG सबसे अच्छी दिखती है।
        </p>
        <label className="block">
          <span className={`block text-center py-2.5 rounded-xl text-sm font-semibold border-2 ${fleetHas ? "border-emerald-600 text-emerald-400" : "border-yellow-600 text-yellow-400"}`}>
            {fleetHas ? "✅ चढ़ी हुई है — बदलने के लिए दबाइए" : "📤 तस्वीर चढ़ाइए"}
          </span>
          <input type="file" accept="image/*" className="hidden"
            onChange={(e) => { uploadFleet(e.target.files?.[0]); e.target.value = ""; }} />
        </label>
        {fleetBusy && <p className="text-[11px] text-neutral-400 text-center">{fleetBusy}</p>}
      </div>
