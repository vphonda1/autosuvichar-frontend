...
// ⚠️ server कभी-कभी JSON की जगह HTML भेज देता है (जैसे file बहुत बड़ी हो)।
//    तब JSON.parse टूटकर "Unexpected token '<'" जैसा गूढ़ सन्देश देता था।
...
async function jsonSafe(r) {
  ...
  try { return JSON.parse(t); }
    ...
    if (/^\s*<!DOCTYPE|^\s*<html/i.test(t)) {
      throw new Error(
        r.status === 413 ? "file बहुत बड़ी है — छोटी करके डालिए"
...
function MusicTab({ apiBase, auth, brandId, accent, flash }) {
  ...
  function load() {
    fetch(`${apiBase.replace(/\/+$/, "")}/api/music/tracks?brand=${brandId}`, { headers: auth })
      .then(jsonSafe).then(setData).catch((e) => flash(e.message, true));
  }
  ...
  async function upload() {
    ...
    try {
      ...
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/music/upload`, { method: "POST", headers: auth, body: fd });
  ...
  async function del(id) {
    ...
    vib([20, 30, 20]);
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/music/tracks/${id}`, { method: "DELETE", headers: auth });
      load();
    } catch (e) { flash(e.message, true); }
  }
  ...
  async function changeMood(id, m) {
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/music/tracks/${id}`, {
...
function ArtTab({ apiBase, auth, brandId, accent, flash }) {
  ...
  function load() {
    fetch(`${apiBase.replace(/\/+$/, "")}/api/festive/themes?brand=${brandId}`, { headers: auth })
      .then(jsonSafe).then(setData).catch((e) => flash(e.message, true));
  }
  useEffect(load, [brandId]);   // eslint-disable-line
  ...
  async function pick(e) {
    ...
    try {
      const fd = new FormData();
      fd.append("art", f);
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/festive/art/${target}`, { method: "POST", headers: auth, body: fd });
      const d = await jsonSafe(r);
  ...
  async function del(id) {
    ...
    vib([20, 30, 20]);
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/festive/art/${id}`, { method: "DELETE", headers: auth });
      load();
    } catch (e) { flash(e.message, true); }
  }
          ...
          <div key={t.id} className="rounded-xl bg-neutral-900 border border-neutral-800 p-2.5">
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-lg bg-neutral-800 border border-neutral-700 flex items-center justify-center overflow-hidden flex-shrink-0">
                {t.hasArt
                  ? <img src={`${apiBase.replace(/\/+$/, "")}/festive-art/${t.id}`} alt="" className="w-full h-full object-contain" />
