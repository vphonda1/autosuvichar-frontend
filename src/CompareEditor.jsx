...
export default function CompareEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  const render = useCallback(() => {
    ...
    const RW = W - RX - 46;
    const BAR = 190;                   // नीचे की पट्टी — तीन लाइनों के लिए
    ...
    c.save();
    c.fillStyle = "#FFFFFF"; c.fillRect(0, by, W, BAR);
    ...
    c.quadraticCurveTo(16, by + BAR * 0.5, 26, H);
    ...
    c.quadraticCurveTo(W - 16, by + BAR * 0.5, W - 26, H);
    ...
    if (shopAddr) {
      c.fillStyle = "#333";
      c.font = `600 ${fit("पता: " + shopAddr, W - LX - 60, 19, 600)}px ${B}`;
      c.fillText("पता: " + shopAddr, LX, ly);
    }
  ...
  async function banaoBg() {
    vib(40); setBgBusy(true); setNote("AI से पृष्ठभूमि बन रही है… (20-40 सेकंड)");
    try {
      const r = await fetch(apiBase + "/api/ai-bg", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
        body: JSON.stringify({
          brand: brandId, w: 1080, h: 1350,
          prompt: "clean modern automobile showroom backdrop, soft city skyline blur, " +
                  "bright airy daylight, subtle geometric pattern on left side, empty space, " +
                  "professional advertising background",
        }),
      });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "नहीं बनी");
      setBgImg(d.dataUrl);
      setNote(`✅ पृष्ठभूमि लग गई (${d.source})`);
