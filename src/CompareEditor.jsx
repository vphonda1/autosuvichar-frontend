...
import { useImageCache, roundRect, drawFit,
         bevelBar, solidText, ribbon, groundShadow } from "./canvasKit.js";
...
export default function CompareEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  const render = useCallback(() => {
    ...
    // ── नीचे की पट्टी — अब उभरी हुई ─────────────────────────
    //  ⚠️ पहले सादा काला डिब्बा था। अब ऊपर चमक, नीचे गहराई, और ऊपर
    //     brand के रंग की पतली रेखा — जैसे छपे हुए poster में होती है।
    const by = H - BAR;
    bevelBar(c, 0, by, W, BAR, { top: "#2a2a2a", bottom: "#0b0b0b", shine: 0.18 });
    bevelBar(c, 0, by - 7, W, 7, { top: S.accent, bottom: S.dark, shine: 0.35 });
    c.fillStyle = "#fff";
    const snp = fit(shopName, W * 0.5, 38, 900);
    solidText(c, shopName, L, by + 52, {
      font: `900 ${snp}px ${D}`, fill: "#ffffff", fill2: "#d8d8d8",
      shadow: "rgba(0,0,0,0.6)", shadowY: 3, shadowBlur: 5,
    });
    c.fillStyle = "#9E9E9E";
    ...
    drawBothLogos(c, ownerRef, logoRef, brandId, W, 26, 84);
      ...
      v1, v2, strip, stripText, showStrip, skin, brandId, bgImg, logoTick, ownerTick, imgCache.tick]);
