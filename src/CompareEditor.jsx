...
export default function CompareEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  const render = useCallback(() => {
    ...
    const RW = W - RX - 46;
    const BAR = 132;                   // नीचे की पट्टी — फ़ोन के डिब्बे के लिए जगह
    ...
    const slots = useStrip
      ? [{ v: v1, top: 88, h: 340 }, { v: v2, top: 500, h: 350 }]
      : [{ v: v1, top: 92, h: 400 }, { v: v2, top: 600, h: 420 }];
    ...
    // ── नीचे की पट्टी — आपके नमूने जैसी ────────────────────
    //  ⚠️ पहले सादा काला डिब्बा था। अब सफ़ेद पट्टी, बाएँ brand के रंग की
    //     तिरछी धार, स्थान का निशान, और दाएँ फ़ोन का गोल डिब्बा।
    const by = H - BAR;
    c.save();
    c.fillStyle = "#FFFFFF"; c.fillRect(0, by, W, BAR);

    // ऊपर दो रंग की पतली रेखा
    bevelBar(c, 0, by - 6, W * 0.55, 6, { top: S.accent, bottom: S.dark, shine: 0.35 });
    bevelBar(c, W * 0.55, by - 6, W * 0.45, 6, { top: "#0B7A3B", bottom: "#064d25", shine: 0.35 });

    // बाईं तिरछी धार — रंग की लहर
    c.beginPath();
    c.moveTo(0, by); c.lineTo(W * 0.30, by);
    c.quadraticCurveTo(W * 0.22, by + BAR * 0.5, W * 0.30, by + BAR);
    c.lineTo(0, by + BAR); c.closePath();
    const gsw = c.createLinearGradient(0, by, W * 0.30, by + BAR);
    gsw.addColorStop(0, S.accent); gsw.addColorStop(1, S.dark);
    c.fillStyle = gsw; c.fill();

    // दाईं हरी लहर
    c.beginPath();
    c.moveTo(W, by); c.lineTo(W * 0.70, by);
    c.quadraticCurveTo(W * 0.78, by + BAR * 0.5, W * 0.70, by + BAR);
    c.lineTo(W, by + BAR); c.closePath();
    const gsw2 = c.createLinearGradient(W, by, W * 0.70, by + BAR);
    gsw2.addColorStop(0, "#0B7A3B"); gsw2.addColorStop(1, "#064d25");
    c.fillStyle = gsw2; c.fill();
    c.restore();

    // स्थान का निशान
    const pinX = L + 6, pinY = by + 74;
    c.save();
    c.fillStyle = S.accent;
    c.beginPath();
    c.moveTo(pinX, pinY + 12);
    c.bezierCurveTo(pinX - 11, pinY - 4, pinX - 9, pinY - 18, pinX, pinY - 18);
    c.bezierCurveTo(pinX + 9, pinY - 18, pinX + 11, pinY - 4, pinX, pinY + 12);
    c.closePath(); c.fill();
    c.fillStyle = "#fff";
    c.beginPath(); c.arc(pinX, pinY - 9, 4, 0, 7); c.fill();
    c.restore();

    c.fillStyle = "#fff";
    const snp = fit(shopName, W * 0.42, 44, 800);
    solidText(c, shopName, L, by + 50, {
      font: `800 ${snp}px ${D}`, fill: S.accent, fill2: S.dark,
      shadow: "rgba(0,0,0,0.20)", shadowY: 2, shadowBlur: 3,
    });
    c.fillStyle = "#444";
    c.font = `600 ${fit(shopSub, W * 0.42, 18, 600)}px ${B}`;
    c.fillText(shopSub, L + 20, by + 82);

    // फ़ोन — सफ़ेद गोल डिब्बे में, जैसे नमूने में है
    const phones = String(phone).split(/[\n,]/).map((x) => x.trim()).filter(Boolean).slice(0, 2);
    const pbW = 330, pbH = BAR - 30, pbX = W - pbW - 30, pbY = by + 15;
    c.save();
    c.fillStyle = "#FFFFFF";
    roundRect(c, pbX, pbY, pbW, pbH, pbH / 2); c.fill();
    c.strokeStyle = "#0B7A3B"; c.lineWidth = 2.5;
    roundRect(c, pbX, pbY, pbW, pbH, pbH / 2); c.stroke();
    // फ़ोन का निशान
    c.fillStyle = "#0B7A3B";
    c.beginPath(); c.arc(pbX + 34, pbY + pbH / 2, 17, 0, 7); c.fill();
    c.fillStyle = "#fff"; c.font = `700 17px system-ui`; c.textAlign = "center";
    c.fillText("✆", pbX + 34, pbY + pbH / 2 + 6);
    c.restore();

    c.textAlign = "left";
    c.fillStyle = "#0B7A3B"; c.font = `700 15px ${B}`;
    c.fillText("Call Us :", pbX + 62, pbY + 22);
    c.fillStyle = "#111";
    c.font = `900 ${phones.length > 1 ? 27 : 34}px system-ui`;
    phones.forEach((p, i) => c.fillText(p, pbX + 62, pbY + (phones.length > 1 ? 46 + i * 30 : 56)));
