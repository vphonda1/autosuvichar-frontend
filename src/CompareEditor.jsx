...
export default function CompareEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  const render = useCallback(() => {
    ...
    if (bg) {
      ...
      c.fillRect(0, 0, W * 0.56, H);
      const g = c.createLinearGradient(W * 0.56, 0, W, 0);
      g.addColorStop(0, "rgba(255,255,255,0.80)");
      g.addColorStop(1, "rgba(255,255,255,0.30)");
      c.fillStyle = g; c.fillRect(W * 0.56, 0, W * 0.44, H);
    }
    ...
    const RW = W - RX - 46;
    const BAR = 190;                   // नीचे की सफ़ेद पट्टी (नाम + जगह + फ़ोन)
    const ADDR_H = 40;                 // उसके नीचे पते की अलग रंगीन पट्टी
    ...
    tRows.forEach((r, i) => {
      ...
      c.font = `${hl ? 800 : 600} ${hl ? 30 : 27}px ${B}`;
    ...
    if (useStrip) {
      ...
      // नीचे रिबन
      if (stripText) {
        const rw = Math.min(sw * 0.86, 300), rh = 40;
        const rx2 = sx + (sw - rw) / 2, ry2 = sy + sh - rh / 2;
    ...
    // ══════════════════════════════════════════════════════════
    //  नीचे की पट्टी — आपके नमूने की तरह तीन लाइनों में
    // ══════════════════════════════════════════════════════════
    //  ⚠️ पिछली बार गड़बड़ हो गई थी: लाल तिरछी लहर ठीक दुकान के नाम के
    //     नीचे आ रही थी और नाम भी लाल था — इसलिए पढ़ा ही नहीं जाता था।
    //     और पते (Address) का कोई ख़ाना ही नहीं था।
    //
    //     अब पट्टी तीन साफ़ हिस्सों में बँटी है और हर हिस्से की जगह नाप कर
    //     तय है — कुछ किसी के ऊपर नहीं चढ़ता:
    //       बायाँ  0 → 62%   : दुकान का नाम, जगह, पता
    //       दायाँ 64% → 97%  : फ़ोन का सफ़ेद डिब्बा
    const by = H - BAR - (shopAddr ? ADDR_H : 0);
    const LZ = W * 0.62;                 // बाएँ हिस्से की हद

    c.save();
    c.fillStyle = "#FFFFFF"; c.fillRect(0, by, W, BAR + (shopAddr ? ADDR_H : 0));

    // ऊपर दो रंग की पतली रेखा
    bevelBar(c, 0, by - 6, LZ, 6, { top: S.accent, bottom: S.dark, shine: 0.35 });
    bevelBar(c, LZ, by - 6, W - LZ, 6, { top: "#0B7A3B", bottom: "#064d25", shine: 0.35 });

    // बाएँ किनारे पतली रंगीन धार — नाम के नीचे नहीं, बिल्कुल किनारे पर
    const gL = c.createLinearGradient(0, by, 0, H);
    gL.addColorStop(0, S.accent); gL.addColorStop(1, S.dark);
    c.fillStyle = gL;
    c.beginPath();
    c.moveTo(0, by); c.lineTo(26, by);
    c.quadraticCurveTo(16, by + BAR * 0.5, 26, by + BAR);
    c.lineTo(0, by + BAR); c.closePath(); c.fill();

    // दाएँ किनारे हरी धार
    const gR = c.createLinearGradient(W, by, W, H);
    gR.addColorStop(0, "#0B7A3B"); gR.addColorStop(1, "#064d25");
    c.fillStyle = gR;
    c.beginPath();
    c.moveTo(W, by); c.lineTo(W - 26, by);
    c.quadraticCurveTo(W - 16, by + BAR * 0.5, W - 26, by + BAR);
    c.lineTo(W, by + BAR); c.closePath(); c.fill();
    c.restore();
    ...
    // दुकान का नाम — सफ़ेद पर, इसलिए साफ़ पढ़ा जाएगा
    const snp = fit(shopName, LZ - LX - 30, 46, 800);
    solidText(c, shopName, LX, ly, {
      font: `800 ${snp}px ${D}`, fill: S.accent, fill2: S.dark,
      shadow: "rgba(0,0,0,0.18)", shadowY: 2, shadowBlur: 3,
    });
    ly += 34;

    // 📍 जगह — दोनों तरफ़ लकीर के साथ
    if (shopPlace) {
      const pp = fit(shopPlace, LZ - LX - 90, 24, 800);
      c.font = `800 ${pp}px ${D}`;
      const pw = c.measureText(shopPlace).width;
      // स्थान का निशान
      c.save();
      c.fillStyle = S.accent;
      const px2 = LX + 11, py2 = ly - 7;
      c.beginPath();
      c.moveTo(px2, py2 + 11);
      c.bezierCurveTo(px2 - 10, py2 - 4, px2 - 8, py2 - 16, px2, py2 - 16);
      c.bezierCurveTo(px2 + 8, py2 - 16, px2 + 10, py2 - 4, px2, py2 + 11);
      c.closePath(); c.fill();
      c.fillStyle = "#fff";
      c.beginPath(); c.arc(px2, py2 - 8, 3.4, 0, 7); c.fill();
      c.restore();

      c.fillStyle = "#111"; c.fillText(shopPlace, LX + 28, ly);
      // दाईं तरफ़ लकीर
      c.strokeStyle = "#333"; c.lineWidth = 2.5;
      c.beginPath();
      c.moveTo(LX + 36 + pw, ly - 8); c.lineTo(Math.min(LZ - 20, LX + 36 + pw + 90), ly - 8);
      c.stroke();
      ly += 30;
    }

    // (पता नीचे अपनी अलग रंगीन पट्टी में जाता है — यहाँ नहीं)

    // ── दायाँ हिस्सा — फ़ोन का डिब्बा ────────────────────────
    const phones = String(phone).split(/[\n,]/).map((x) => x.trim()).filter(Boolean).slice(0, 2);
    const pbX = LZ + 14, pbW = W - pbX - 40;
    const pbH = phones.length > 1 ? 108 : 84;
    const pbY = by + (BAR - pbH) / 2;

    c.save();
    c.fillStyle = "#FFFFFF";
    roundRect(c, pbX, pbY, pbW, pbH, pbH / 2); c.fill();
    c.strokeStyle = "#0B7A3B"; c.lineWidth = 3;
    roundRect(c, pbX, pbY, pbW, pbH, pbH / 2); c.stroke();

    // फ़ोन का गोल निशान
    const icx = pbX + pbH / 2 + 2, icy = pbY + pbH / 2;
    c.fillStyle = "#0B7A3B";
    c.beginPath(); c.arc(icx, icy, pbH * 0.30, 0, 7); c.fill();
    c.fillStyle = "#fff";
    c.font = `700 ${Math.round(pbH * 0.30)}px system-ui`;
    c.textAlign = "center"; c.textBaseline = "middle";
    c.fillText("✆", icx, icy + 1);
    c.textAlign = "left"; c.textBaseline = "alphabetic";
    c.restore();

    const tx = icx + pbH * 0.34 + 14;
    c.fillStyle = "#0B7A3B"; c.font = `700 15px ${B}`;
    c.fillText("Call Us :", tx, pbY + 26);
    c.fillStyle = "#111";
    const php = phones.length > 1 ? 28 : 34;
    c.font = `900 ${php}px system-ui`;
    phones.forEach((p, i) => c.fillText(p, tx, pbY + (phones.length > 1 ? 56 + i * 32 : 62)));

    // ── पते की अपनी पट्टी ───────────────────────────────────
    //  ⚠️ पहले पता उसी सफ़ेद पट्टी में एक ही रंग में लिख दिया था — आपके
    //     नमूने में वह अलग रंगीन पट्टी में है, इसलिए साफ़ अलग दिखता है।
    if (shopAddr) {
      const abH = 40, abY = H - abH;
      const gA = c.createLinearGradient(0, abY, W, abY);
      gA.addColorStop(0, S.accent);
      gA.addColorStop(0.55, S.dark);
      gA.addColorStop(1, "#0B7A3B");
      c.fillStyle = gA; c.fillRect(0, abY, W, abH);

      // ऊपर हल्की चमक
      c.save(); c.globalAlpha = 0.18; c.fillStyle = "#fff";
      c.fillRect(0, abY, W, 6); c.restore();

      // छोटा स्थान निशान
      c.save();
      c.fillStyle = "#fff";
      const ax = LX + 8, ay = abY + abH / 2;
      c.beginPath();
      c.moveTo(ax, ay + 8);
      c.bezierCurveTo(ax - 7, ay - 3, ax - 6, ay - 12, ax, ay - 12);
      c.bezierCurveTo(ax + 6, ay - 12, ax + 7, ay - 3, ax, ay + 8);
      c.closePath(); c.fill();
      c.restore();

      c.fillStyle = "#fff";
      c.font = `700 ${fit("पता : " + shopAddr, W - LX - 90, 19, 700)}px ${B}`;
      c.textBaseline = "middle";
      c.fillText("पता : " + shopAddr, LX + 24, abY + abH / 2 + 1);
  ...
  async function banaoBg() {
    ...
    try {
      ...
      setBgImg(d.dataUrl);
      setBgMsg(`✅ पृष्ठभूमि लग गई${d.source ? ` (${d.source})` : ""} — पसंद न आए तो दोबारा दबाइए`);
      vib([30, 40, 60]);
    } catch (e) {
      setBgMsg("");
      setBgErr(e.message);
  ...
  return (
    <div className="space-y-3 pb-10">
      ...
      <div className="rounded-xl bg-neutral-950 border border-neutral-800 p-3 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button type="button" onClick={() => { vib(15); setBgImg(""); }}
