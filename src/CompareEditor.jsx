...
export default function CompareEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  useEffect(() => {
    fetch(`${apiBase.replace(/\/+$/, "")}/api/bg?brand=${brandId}`, { headers: { Authorization: "Bearer " + token } })
  ...
  async function sahejo() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/bg/save`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
        body: JSON.stringify({
  ...
  function lagao(b) {
    ...
    setBgErr("");
    fetch(`${apiBase.replace(/\/+$/, "")}/api/bg/${b._id}/used`, {
  ...
  async function banaoBg() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/ai-bg", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
        body: JSON.stringify({
