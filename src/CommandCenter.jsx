...
export default function CommandCenter({ apiBase, token, brandId, accent = "#E4002B", onDone, forcedType, hideOwnInput, sharedText, onTextChange, autoTrigger, tryAIBg }) {
  ...
  // ⚠️ पहले सिर्फ़ अपने भीतरी "txt" state पर निर्भर था। अब बाहर से (नए
  //    एक-जगह-वाले panel से) भी text/प्रकार सीधे भेजे जा सकते हैं — इससे
  //    "state अभी update नहीं हुई" वाली दौड़ की गड़बड़ी नहीं होती।
  async function banao(overrideText, overrideType) {
    const useTxt = overrideText !== undefined ? overrideText : txt;
    if (!useTxt.trim()) return;
    vib(50); setBusy(true); setErr(""); setCamp(null); setOpen(null);
    if (pollRef.current) clearInterval(pollRef.current);

    try {
      const body = { command: useTxt.trim(), brand: brandId };
      const ft = overrideType !== undefined ? overrideType : forcedType;
      if (ft) body.forceType = ft;
      if (tryAIBg) body.tryAIBg = true;
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/campaign/one-in`, {
      ...
      pollRef.current = setInterval(async () => {
        n++;
        try {
          const rr = await fetch(`${apiBase.replace(/\/+$/, "")}/api/campaign/one-in/${d.id}`, { headers: auth });
  ...
  // बाहर के panel से "बनाओ" दबाने पर — triggerKey बदलते ही यह चल पड़े
  const lastTrigger = useRef(0);
  useEffect(() => {
    if (autoTrigger && autoTrigger > lastTrigger.current) {
      lastTrigger.current = autoTrigger;
      setTxt(sharedText || "");
      banao(sharedText || "", forcedType);
    }
  ...
  async function bhejo() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/campaign/one-in/${camp.id}/publish`, {
        method: "POST", headers: auth,
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── ऊपर: एक ही डिब्बा — बाहर से control हो तो छिप जाए ──── */}
      {!hideOwnInput && (
