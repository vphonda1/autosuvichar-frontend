...
// ⚠️ पहले "API_BASE + u" सीधे जोड़ा जाता था — अगर Render में PUBLIC_URL के
//    आख़िर में slash हो (जैसा पहले भी एक बार मिल चुका है), तो पता
//    "...com//generated/xxx.png" बन जाता — दो slash से Express या browser
//    उसे पहचानते नहीं, तस्वीर टूटी हुई दिखती (जैसा screenshot में दिखा)।
export const media = (u) => (u && u.startsWith("/") ? API_BASE.replace(/\/+$/, "") + u : u);
...
export async function api(p, opts = {}) {
  // ⚠️ यह पूरे app की हर API call के लिए इस्तेमाल होता है — अगर Render में
  //    PUBLIC_URL के आख़िर में slash हो, तो हर request "...com//api/..."
  //    बन जाती, जो Express किसी भी route से मेल नहीं खिलाता।
...
export async function nativeShare(item, brandName = "") {
  ...
  // ⚠️ whatsappText — अगर बनी हो — में *bold*/_italic_ निशान होते हैं जो
  //    WhatsApp में सजकर दिखते हैं। item.text (poster पर छपने वाला सादा
  //    वाला) सिर्फ़ तभी इस्तेमाल हो जब whatsappText ना बनी हो (पुराने posts,
  //    या जिनमें यह feature लागू होने से पहले बनी थीं)।
  const baseText = item.whatsappText || item.text || "";
  const text = baseText + (item.customerName ? `\n${item.customerName} — ${item.bikeName || ""}` : "");
  if (!navigator.share) {
    window.open("https://wa.me/?text=" + encodeURIComponent(text + (imgUrl ? "\n" + imgUrl : "")), "_blank");
    return;
  }
  //  ⚠️ Facebook और Instagram की Story सिर्फ़ तस्वीर लेती है — साथ भेजा
  //     हुआ caption वो चुपचाप छोड़ देती हैं (WhatsApp ले लेता है, इसलिए
  //     वहाँ दिखता था)। इसका कोई तकनीकी रास्ता नहीं कि app ज़बरदस्ती
  //     caption डाल दे — यह उन्हीं का नियम है।
  //     इसलिए share खुलने से *पहले* caption आपके clipboard में रख देते
  //     हैं — Story खुलते ही "Aa"/text वाले खाने में एक बार paste (देर तक
  //     दबाकर) कीजिए, पूरा caption आ जाएगा।
  let copied = false;
  try {
  ...
  try {
    if (imgUrl) {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        if (copied) setTimeout(() => alert("📋 Caption copy हो गया है।\nFacebook/Instagram Story में text वाले खाने पर देर तक दबाकर Paste कर दीजिए।"), 600);
        return;
      }
    }
