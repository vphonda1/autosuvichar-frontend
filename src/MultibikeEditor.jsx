...
export default function MultibikeEditor({ apiBase, token, brandId, onSent }) {
  ...
  const render = useCallback(() => {
    ...
    if (headerStyle === "oval_white") {
      ...
      ctx.fillStyle = rg; ctx.beginPath(); ctx.roundRect(r2x, r2y, r2w, r2h, 8); ctx.fill();
      ...
      ctx.fillStyle=rg2; ctx.beginPath(); ctx.roundRect(rx,ry,rw,rh,14); ctx.fill();
      ctx.strokeStyle="#fff"; ctx.lineWidth=3; ctx.beginPath(); ctx.roundRect(rx+5,ry+5,rw-10,rh-10,10); ctx.stroke();
    ...
    activeBikes.forEach((bk, i) => {
      ...
      if (bk.img) {
        ...
        if (im.complete) { ctx.drawImage(im, bx, bkY, bw, bkH); }
      ...
      // Shadow
      ctx.fillStyle=isDark?"#000":"#ccc"; ctx.beginPath(); ctx.roundRect(px2+4,py2+5,pw,ph,8); ctx.fill();
      // Box
      const pgg=ctx.createLinearGradient(px2,py2,px2,py2+ph);
      pgg.addColorStop(0,lighten(pbObj.bg,20)); pgg.addColorStop(1,pbObj.bg);
      ctx.fillStyle=pgg; ctx.beginPath(); ctx.roundRect(px2,py2,pw,ph,8); ctx.fill();
      ctx.fillStyle="rgba(255,255,255,.18)"; ctx.beginPath(); ctx.roundRect(px2+3,py2+3,pw-6,ph*.45,[8,8,0,0]); ctx.fill();
      ctx.strokeStyle=pbObj.bg===pbObj.text?"#fff":lighten(pbObj.bg,50); ctx.lineWidth=2;
      ctx.beginPath(); ctx.roundRect(px2,py2,pw,ph,8); ctx.stroke();
      ...
      ctx.fillStyle=isDark?"#000":"#ccc"; ctx.beginPath(); ctx.roundRect(cx2+3,cy2+4,cw,ch,16); ctx.fill();
      ...
      ctx.fillStyle=cgg; ctx.beginPath(); ctx.roundRect(cx2,cy2,cw,ch,16); ctx.fill();
      ctx.fillStyle="rgba(255,255,255,.2)"; ctx.beginPath(); ctx.roundRect(cx2+3,cy2+3,cw-6,ch*.45,[16,16,0,0]); ctx.fill();
