...
export default function MultibikeEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  //  ⚠️ command-router.js draft.bikes में {img, name, price, emi} भेजता है,
  //     पर यहाँ की bikes सूची {img, name, exPrice, cashback} माँगती है —
  //     दो अलग नाम। बिना मिलाए रख देने पर हर price ख़ाली दिखता। यहीं मिलाते हैं।
  useEffect(() => {
    if (!draft) return;
    if (draft.headline) { const [h1, ...rest] = String(draft.headline).split("\n"); setHeadline1(h1 || ""); if (rest.length) setHeadline2(rest.join(" ")); }
    if (draft.shopName) { setDealer(draft.shopName); setTouched(true); }
    if (draft.shopSub) setDealerSub(draft.shopSub);
    if (draft.phone) setPhone(draft.phone);
    if (Array.isArray(draft.bikes) && draft.bikes.length) {
      setBikes(draft.bikes.slice(0, 4).map((b) => ({
        id: uid(), name: b.name || "", img: b.img || null,
        exPrice: b.price ? `${b.price}/-*` : "",
        cashback: b.emi ? `EMI ${b.emi}` : "",
        cashLabel: b.emi ? "" : "Instant Cashback",
        enabled: true,
      })));
    }
  }, [draft]);
