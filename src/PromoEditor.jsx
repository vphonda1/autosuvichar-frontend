...
export default function PromoEditor({ apiBase, token, brandId, onSent }) {
  ...
  const inner = `
    ...
    <rect x="0" y="806" width="${W}" height="48" fill="${ACCENT}"/><text x="540" y="838" text-anchor="middle" font-family="Noto Sans Devanagari,Arial,sans-serif" font-size="26" font-weight="700" fill="#fff">${esc(feats)}</text>
    <rect x="0" y="856" width="${W}" height="54" fill="#0d0d0d"/><rect x="0" y="856" width="${W}" height="3" fill="${GOLD}"/>
    <text x="540" y="881" text-anchor="middle" font-family="Noto Sans Devanagari,Arial,sans-serif" font-size="23" font-weight="700" fill="${GOLD}">कम ब्याज पर फाइनेंस सुविधा उपलब्ध</text>
    <text x="540" y="903" text-anchor="middle" font-family="Arial,sans-serif" font-size="20" fill="#fff">${esc(cBanks.join("   •   "))}</text>
