  ...
  return (
    <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-3 mb-5 space-y-3">
      ...
      <div>
        ...
        <span className="text-xs text-neutral-400 block mt-2">आकर्षक बटन/बैज जोड़ें (text बदल सकते हैं — "|" से दो लाइन)</span>
        <div className="flex flex-wrap gap-2 mt-1">
          {BADGES.map((b) => (
            <button key={b.style} type="button" title={b.label} onClick={() => addBadge(b.style, b.def)} className="bg-neutral-800 border border-neutral-700 rounded-lg p-1" style={{ width: 96, height: 40 }}
              dangerouslySetInnerHTML={{ __html: `<svg viewBox="-185 -60 370 120" width="86" height="30" style="display:block;margin:auto">${buildBadge(b.style, b.def)}</svg>` }} />
          ))}
        </div>
        {selItem && (
