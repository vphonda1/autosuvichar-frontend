...
export default function PromoEditor({ apiBase, token, brandId, onSent }) {
  ...
  const inner = `
    ...
    ${chipRow(f.features.split(",").map((s) => s.trim()).filter(Boolean), 802, 44, W - 40, W)}
    <text x="540" y="884" text-anchor="middle" font-family="Noto Sans Devanagari,Arial,sans-serif" font-size="24" font-weight="800" fill="${GOLD}">कम ब्याज पर फाइनेंस सुविधा उपलब्ध</text>
    ${chipRow(cBanks, 894, 38, W - 40, W)}
