...
export default function App() {
  ...
  async function doGenerate() { setErr(""); setBusy(true); try { await api("/api/generate", { method: "POST", body: JSON.stringify({ brand: brandId, type, offer: cOffers.join(","), sticker: cStickers.join(","), decor: cDecor.join(","), bg: cBg, design: cDesign, autoDecor: cAutoDecor, autoSeed: Date.now() + "-" + Math.random(), tags: cTags.join(" "), festival: type === "festival" ? cFestival : undefined }) }); await load(); } catch (e) { setErr(e.message); } setBusy(false); }
