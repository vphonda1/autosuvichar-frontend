...
export default function App() {
  ...
  async function load() {
    try {
      ...
      if (tab === "delivery") { const [dp, ds] = await Promise.all([api(`/api/deliveries?brand=${brandId}&status=pending`), api(`/api/deliveries?brand=${brandId}&status=sent`)]); setDeliv(dp); setSentDeliv(ds.slice(0, 10)); }
    ...
    <div style={{ fontFamily: "system-ui" }} className="min-h-screen bg-neutral-950 text-neutral-100 px-4 py-5">
      <div className="max-w-3xl mx-auto">
        ...
        {tab === "content" && (<>
          ...
          <div className="space-y-3 mb-8">{pending.map((p) => (
            <div key={p._id} className="rounded-2xl bg-neutral-900 border border-neutral-800 overflow-hidden">
              ...
              {!p.video && (p.imgUrl || p.images?.square) && <a href={media(p.imgUrl || p.images?.square)} download={`vphonda-${p._id}.png`} target="_blank" rel="noreferrer" className="block text-center text-xs py-2 bg-neutral-800 text-neutral-200 border-t border-neutral-700">⬇ Download फोटो</a>}
              <div className="p-3">
                ...
                {/* 📤 Native Share — phone का share menu, कोई API नहीं */}
                <button onClick={() => nativeShare(p, API_BASE)} className="w-full rounded-lg py-2.5 text-sm font-semibold mb-2 border-2 flex items-center justify-center gap-2" style={{ borderColor: brand.accent, color: brand.accent, background: "transparent" }}>
                  📤 Share करें (WhatsApp / Facebook / Instagram…)
                </button>
          ...
          <div className="space-y-2 pb-10">{sent.length === 0 && <Empty>अभी कुछ नहीं भेजा गया।</Empty>}{sent.map((s) => <SentRow key={s._id} s={s} kind="content" onResend={resend} onDel={del} busy={busy} />)}</div>
        </>)}
        ...
        {tab === "delivery" && (<>
          ...
          <div className="space-y-3 pb-10">{deliv.map((d) => (
            <div key={d._id} className="rounded-2xl bg-neutral-900 border border-neutral-800 overflow-hidden">
              ...
              <div className="p-3">
                ...
                {ADMIN.concat("manager").includes(user.role)
                  ? <>
                      <button onClick={() => nativeShare(d, API_BASE)} className="w-full rounded-lg py-2.5 text-sm font-semibold mb-2 border-2 flex items-center justify-center gap-2" style={{ borderColor: brand.accent, color: brand.accent, background: "transparent" }}>
                        📤 Share करें (WhatsApp / Facebook / Instagram…)
                      </button>
          ...
          <div className="space-y-2 pb-10">{sentDeliv.length === 0 && <Empty>अभी कोई delivery नहीं भेजी।</Empty>}{sentDeliv.map((s) => <SentRow key={s._id} s={s} kind="delivery" onResend={resend} onDel={del} busy={busy} />)}</div>
        </>)}
        {tab === "leads" && (<>
...
// Native share — phone का built-in share menu (WhatsApp, FB, Instagram सब एक जगह)
async function nativeShare(item, apiBase) {
  const imgUrl = item.imgUrl ? `${apiBase}${item.imgUrl}` : (item.images?.square ? `${apiBase}${item.images.square}` : null);
  const text = item.text || "";
  // Web Share API (mobile पर works)
  if (navigator.share) {
    try {
      if (imgUrl && navigator.canShare && navigator.canShare({ files: [] })) {
        // Image fetch करके file share करो
        const resp = await fetch(imgUrl);
        const blob = await resp.blob();
        const file = new File([blob], "vphonda.png", { type: "image/png" });
        if (navigator.canShare({ files: [file] })) {
          await navigator.share({ files: [file], text, title: "VP Honda" });
          return;
        }
      }
      // Fallback: text + URL share
      await navigator.share({ text: text + (imgUrl ? "\n\n" + imgUrl : ""), title: "VP Honda" });
    } catch (e) { if (e.name !== "AbortError") alert("Share नहीं हो सका: " + e.message); }
  } else {
...
const SentRow = ({ s, kind, onResend, onDel, busy }) => (
  <div className="rounded-xl bg-neutral-900/60 border border-neutral-800 px-3 py-2.5">
    <div className="flex gap-2">
      {(s.imgUrl || (s.images && s.images.square)) && <img src={media(s.imgUrl || s.images.square)} alt="" className="w-16 h-16 rounded-lg object-cover border border-neutral-700 flex-shrink-0" />}
      <div className="flex-1 min-w-0">
        ...
        <div className="flex gap-2 mt-1.5">
          {(s.imgUrl || (s.images && s.images.square)) && <a href={media(s.imgUrl || s.images.square)} target="_blank" rel="noreferrer" className="text-[11px] rounded-lg px-2 py-1 border border-neutral-700 text-neutral-300">👁 खोलें</a>}
          {(s.imgUrl || (s.images && s.images.square)) && <a href={media(s.imgUrl || s.images.square)} download={`vphonda-${s._id}.png`} className="text-[11px] rounded-lg px-2 py-1 border border-neutral-700 text-neutral-300">⬇ download</a>}
          <button type="button" onClick={() => nativeShare(s, API_BASE)} className="text-[11px] rounded-lg px-2 py-1 border border-neutral-600 text-neutral-300">📤 Share</button>
          <button type="button" disabled={busy} onClick={() => onResend(kind, s._id)} className="text-[11px] rounded-lg px-2 py-1 font-semibold text-black" style={{ background: "#ffd400" }}>🔄 दोबारा</button>
          <button type="button" disabled={busy} onClick={() => onDel(kind, s._id)} className="text-[11px] rounded-lg px-2 py-1 border border-red-700 text-red-400">🗑</button>
        </div>
      </div>
    </div>
  </div>
...
function SettingCard({ id, b, cur, onSave, onYT, busy }) {
  ...
  async function findIgId() {
    if (!f.fbPageId || !f.fbToken) { setIgMsg("⚠️ पहले FB Page ID और FB Token भरें"); return; }
    setIgMsg("🔍 ढूँढ रहे हैं...");
    try {
      const r = await fetch(`${API_BASE}/api/ig-account-id?pageId=${f.fbPageId}&token=${f.fbToken}`);
      const d = await r.json();
      if (d.found) { setF({ ...f, igUserId: d.igId }); setIgMsg(`✅ मिला: ${d.igId} (auto-fill हो गया)`); }
      else setIgMsg("❌ " + d.msg);
