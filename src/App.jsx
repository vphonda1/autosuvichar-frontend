...
export default function App() {
  ...
  // manifest shortcut से खुला हो तो (?tab=autoengine) सीधे उसी पेज पर
  const [tab, setTab] = useState(() => {
    try {
      const t = new URLSearchParams(window.location.search).get("tab");
      return t && /^[a-z]+$/.test(t) ? t : "home";
    } catch (_) { return "home"; }
  });
  ...
  const [form, setForm] = useState({ customerName: "", customerMobile: "", bikeName: "", offer: "", file: null, music: "", bg: "auto" });
  ...
  // ग्राहक को उसकी photo सीधे WhatsApp पर भेजो (जो नंबर पोस्ट के साथ है, या अभी पूछो)
  async function sendToCustomer(kind, item) {
    let mob = item.customerMobile || "";
    if (!mob) {
      mob = (window.prompt(`${item.customerName || "ग्राहक"} का WhatsApp नंबर लिखिए`, "") || "").trim();
      if (!mob) return;
    }
    setBusy(true); setErr("");
    try {
      const r = await fetch(`${API_BASE}/api/${kind}/${item._id}/send-customer`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + TOKEN },
        body: JSON.stringify({ customerMobile: mob, force: !!item.sentToCustomerAt }),
      });
      const j = await r.json();
      if (!r.ok) throw new Error((j.error || "Error") + (j.hint ? "\n\n" + j.hint : ""));
      alert(j.message || "भेज दिया");
      await load();
    } catch (e) { setErr(e.message); }
    setBusy(false);
  }
  ...
  async function resend(kind, id) {
    if (!confirm("यह poster दोबारा भेजें?")) return;
    setResendBusy(b => ({ ...b, [id]: true }));
    setResendNote(""); setErr("");
    try {
      const r = await api(`/api/${kind}/${id}/resend`, { method: "POST" });
      await load();
      const ok = (r?.results || []).filter(x => x.ok).map(x => x.platform).join(", ");
      const fail = (r?.results || []).filter(x => !x.ok).map(x => x.platform + ":" + x.error).join(", ");
      setResendNote(ok ? `✅ भेजा: ${ok}` + (fail ? ` | ❌ ${fail}` : "") : `❌ fail: ${fail || "unknown error"}`);
      setTimeout(() => setResendNote(""), 6000);
    } catch (e) { setErr(e.message); }
    setResendBusy(b => ({ ...b, [id]: false }));
  }
  ...
  async function submitDelivery() {
    ...
    setErr(""); setBusy(true);
    try {
      const fd = new FormData(); fd.append("brand", brandId); fd.append("customerName", form.customerName); if (form.customerMobile) fd.append("customerMobile", form.customerMobile); fd.append("bikeName", form.bikeName); fd.append("offer", form.offer); fd.append("bg", form.bg); if (form.music) fd.append("music", form.music); fd.append("photo", form.file);
      const res = await fetch(API_BASE + "/api/delivery", { method: "POST", headers: { Authorization: "Bearer " + TOKEN }, body: fd });
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error || "upload failed");
      setForm({ customerName: "", customerMobile: "", bikeName: "", offer: "", file: null, music: "", bg: "auto" }); await load();
    } catch (e) { setErr(e.message); } setBusy(false);
  }
  ...
  // manifest के shortcuts (?tab=autoengine आदि) से सीधे उसी पेज पर

  const TABS = [["home", "🏠 होम"], ["queue", "📋 Queue"], ["ai", "🤖 AI बनाओ"], ["automkt", "🚀 Auto Marketing"], ["autoengine", "⚡ एक साथ / अपने आप"], ["aidelivery", "📸 AI Delivery"], ["aivideo", "🎬 AI Video"], ["aivoice", "🎙️ AI आवाज़"], ["aistudio", "📱 Platform + Versions"], ["ainews", "📰 खबरें"], ["content", "📝 कंटेंट"], ["promo", "📣 विज्ञापन+"], ["mega", "🔥 Mega Offer"], ["booking", "📋 बुकिंग"], ["multibike", "🏍️ Multi Bike"], ["hiring", "💼 भर्ती"], ["luckydraw", "🎉 Lucky Draw"], ["delivery", "🎥 Delivery"], ["leads", "👥 Leads"], ["analytics", "📊 Analytics"], ["aireport", "🧠 AI Report"], ["vehicles", "🏍️ गाड़ियाँ"], ["memory", "🧠 Brand Memory"], ["automation", "🎛️ Automation"], ["maint", "🧹 रखरखाव"], ["logos", "🖼️ Logo"], ["waapp", "📱 WhatsApp"], ["notif", "🔔"], ...(isAdmin ? [["settings", "⚙️ Settings"]] : [])];

  return (
    <div style={{
      fontFamily: "system-ui",
      // install होने पर ऊपर notch और नीचे home-bar से टकराए नहीं
      paddingTop: "max(1.25rem, env(safe-area-inset-top))",
      paddingBottom: "max(1.25rem, env(safe-area-inset-bottom))",
    }} className="min-h-screen bg-neutral-950 text-neutral-100 px-4">
      <InstallPrompt />
      <div className="max-w-3xl mx-auto">
        ...
        {/* ===== DASHBOARD ===== */}
        {tab === "home" && (
          <Dashboard apiBase={API_BASE} token={TOKEN} brandId={brandId} setTab={setTab} />
        )}

        {/* ===== QUEUE + ACTIVITY ===== */}
        {tab === "queue" && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">📋 सब कुछ एक जगह — क्या कहाँ पहुँचा</h2>
            <Queue apiBase={API_BASE} token={TOKEN} brandId={brandId} setTab={setTab} />
          </div>
        )}
        ...
        {tab === "ai" && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🤖 एक लाइन में बताएं, AI बाकी संभाल लेगा</h2>
            <AICommandCenter apiBase={API_BASE} token={TOKEN} brandId={brandId} onSent={load} />
          </div>
        )}
        ...
        {tab === "automation" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🎛️ कितना auto चले, और खर्च की सीमा</h2>
            <Automation apiBase={API_BASE} token={TOKEN} brandId={brandId} />
          </div>
        )}
        ...
        {tab === "memory" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🧠 Brand Memory — एक बार भरें, AI हमेशा याद रखेगा</h2>
            <BrandMemory apiBase={API_BASE} token={TOKEN} brandId={brandId} />
          </div>
        )}
        ...
        {tab === "vehicles" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🏍️ गाड़ियों की जानकारी — AI इसी से price लेगा</h2>
            <Vehicles apiBase={API_BASE} token={TOKEN} brandId={brandId} />
          </div>
        )}

        {/* ===== AI REPORT ===== */}
        {tab === "aireport" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🧠 AI Report — कैसा चल रहा है, आगे क्या करें</h2>
            <AIReport apiBase={API_BASE} token={TOKEN} brandId={brandId} />
          </div>
        )}
        ...
        {tab === "automkt" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🚀 एक button — पूरे दिन/हफ्ते का marketing</h2>
            <AutoMarketing apiBase={API_BASE} token={TOKEN} brandId={brandId} onSent={load} />
          </div>
        )}
        ...
        {tab === "waapp" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">📱 रोज़ का content सीधे WhatsApp पर — वहीं से भेजो या रोको</h2>
            <WhatsAppApproval apiBase={API_BASE} token={TOKEN} />
          </div>
        )}
        ...
        {tab === "logos" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🖼️ कौन-सा logo कहाँ लगे — सिर्फ़ आपकी अपनी files</h2>
            <LogoSettings apiBase={API_BASE} token={TOKEN} />
          </div>
        )}

        {/* ===== AUTO ENGINE (PRD #23,#29,#33) ===== */}
        {tab === "autoengine" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">⚡ एक साथ कई posts, घटना पर अपने आप, और असली आँकड़े</h2>
            <AutoEngine apiBase={API_BASE} token={TOKEN} brandId={brandId} onSent={load} />
          </div>
        )}
        ...
        {tab === "maint" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🧹 जगह, अटके posts, और स्थायी रिकॉर्ड</h2>
            <Maintenance apiBase={API_BASE} token={TOKEN} brandId={brandId} />
          </div>
        )}
        ...
        {tab === "aidelivery" && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">📸 Photos डालें — AI पूरा delivery post बना देगा</h2>
            <AIDelivery apiBase={API_BASE} token={TOKEN} brandId={brandId} onSent={load} />
          </div>
        )}
            ...
            <div key={p._id} className="rounded-2xl bg-neutral-900 border border-neutral-800 overflow-hidden">
              ...
              {!p.video && (p.imgUrl || p.images?.square) && (
                <div className="flex gap-1 bg-neutral-800 border-t border-neutral-700">
                  ...
                  <button onClick={() => { vib(20); sendToCustomer("content", p); }} disabled={busy}
                    className="flex-1 text-center text-xs py-2 text-neutral-200 border-l border-neutral-700 disabled:opacity-50"
                    title="ग्राहक के WhatsApp पर उनकी photo भेजें">
                    {p.sentToCustomerAt ? "✅ ग्राहक को गई" : "📲 ग्राहक को"}
                  </button>
                </div>
              )}
              <div className="p-3">
                ...
                {ADMIN.concat("manager").includes(user.role) && (
                  <QualityCheck apiBase={API_BASE} token={TOKEN} contentId={p._id} brand={p.brand} text={p.text} onFixed={load} />
                )}
              </div>
            </div>))}
          </div>
          {resendNote && <div className={`rounded-xl px-4 py-3 text-sm font-semibold mb-2 ${resendNote.startsWith("✅") ? "bg-emerald-900/60 text-emerald-300" : "bg-red-900/60 text-red-300"}`}>{resendNote}</div>}
          <Title>भेजे गए (last 10)</Title>
          <div className="space-y-2 pb-10">{sent.length === 0 && <Empty>अभी कुछ नहीं भेजा गया।</Empty>}{sent.map((s) => <SentRow key={s._id} s={s} kind="content" onResend={resend} onDel={del} busy={resendBusy[s._id] || false} apiBase={API_BASE} onView={(s) => window.open(s.imgUrl ? API_BASE + s.imgUrl : "", "_blank")} onEdit={(s) => setTab("editor")} />)}</div>
        </>)}
        ...
        {tab === "booking" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">📋 बुकिंग के फायदे — Festival Booking Poster</h2>
            <BookingEditor apiBase={API_BASE} token={TOKEN} brandId={brandId} onSent={load} />
          </div>
        )}

        {tab === "multibike" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🏍️ Multi Bike Comparison Poster — Mega March Style</h2>
            <MultibikeEditor apiBase={API_BASE} token={TOKEN} brandId={brandId} onSent={load} />
          </div>
        )}

        {tab === "hiring" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">💼 भर्ती पोस्टर — We Are Hiring</h2>
            <HiringEditor apiBase={API_BASE} token={TOKEN} brandId={brandId} onSent={load} />
          </div>
        )}

        {tab === "luckydraw" && isAdmin && (
          <div className="space-y-3">
            <h2 className="text-sm font-semibold text-neutral-400">🎉 Lucky Draw — फेस्टिवल ऑफर पोस्टर</h2>
            <LuckyDrawEditor apiBase={API_BASE} token={TOKEN} brandId={brandId} onSent={load} />
          </div>
        )}
        ...
        {tab === "delivery" && (<>
          ...
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-4 mb-5 space-y-3">
            ...
            <input placeholder="ग्राहक का WhatsApp नंबर (वैकल्पिक)" inputMode="numeric" value={form.customerMobile} onChange={(e) => setForm({ ...form, customerMobile: e.target.value })} className="w-full bg-neutral-800 rounded-lg p-2 text-sm outline-none" />
            ...
            <div key={d._id} className="rounded-2xl bg-neutral-900 border border-neutral-800 overflow-hidden">
              ...
              {!d.video && (d.imgUrl || d.images?.square) && (
                <div className="flex gap-1 bg-neutral-800 border-t border-neutral-700">
                  ...
                  <button onClick={() => { vib(20); sendToCustomer("delivery", d); }} disabled={busy}
                    className="flex-1 text-center text-xs py-2 text-neutral-200 border-l border-neutral-700 disabled:opacity-50">
                    {d.sentToCustomerAt ? "✅ ग्राहक को गई" : "📲 ग्राहक को"}
                  </button>
                </div>
              )}
              <div className="p-3">
          ...
          <div className="space-y-2 pb-10">{sentDeliv.length === 0 && <Empty>अभी कोई delivery नहीं भेजी।</Empty>}{sentDeliv.map((s) => <SentRow key={s._id} s={s} kind="delivery" onResend={resend} onDel={del} busy={resendBusy[s._id] || false} apiBase={API_BASE} onView={(s) => window.open(s.imgUrl ? API_BASE + s.imgUrl : s.video ? API_BASE + s.video : "", "_blank")} onEdit={(s) => setTab("delivery")} />)}</div>
        </>)}
        {tab === "leads" && (<>
