...
export default function App() {
  ...
  const [promoForm, setPromoForm] = useState({ model: "", price: "", downPayment: "", cashback: "", features: "", file: null, bg: "light", vehicle: "", cutout: true, aiPrompt: "", offer: "", sticker: "" });
  ...
  // actions
  async function doGenerate() { setErr(""); setBusy(true); try { await api("/api/generate", { method: "POST", body: JSON.stringify({ brand: brandId, type, offer: cOffer, sticker: cSticker }) }); await load(); } catch (e) { setErr(e.message); } setBusy(false); }
  ...
  async function submitPromo() {
    ...
    try {
      ...
      if (promoForm.sticker) fd.append("sticker", promoForm.sticker);
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error || "upload failed");
      setPromoForm({ model: "", price: "", downPayment: "", cashback: "", features: "", file: null, bg: "light", vehicle: "", cutout: true, aiPrompt: "", offer: "", sticker: "" });
      setTab("content");
    } catch (e) { setErr(e.message); } setBusy(false);
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {tab === "content" && (<>
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-4 mb-5">
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              <label className="text-xs text-neutral-400">Offer
                <select value={cOffer} onChange={(e) => setCOffer(e.target.value)} className="w-full bg-neutral-800 rounded-lg p-2 text-sm border border-neutral-700 mt-1 text-white">{OFFER_OPTS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></label>
              <label className="text-xs text-neutral-400">Sticker design
                <select value={cSticker} onChange={(e) => setCSticker(e.target.value)} className="w-full bg-neutral-800 rounded-lg p-2 text-sm border border-neutral-700 mt-1 text-white">{STICKER_OPTS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></label>
            </div>
        ...
        {tab === "promo" && (
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-4 mb-5 space-y-3">
            ...
            <div>
              <span className="text-xs text-neutral-400">Offer + Sticker design (poster पर seal लगेगा)</span>
              <div className="grid grid-cols-2 gap-2 mt-1">
                <select value={promoForm.offer} onChange={(e) => setPromoForm({ ...promoForm, offer: e.target.value })} className="bg-neutral-800 rounded-lg p-2 text-sm border border-neutral-700 text-white">{OFFER_OPTS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>
                <select value={promoForm.sticker} onChange={(e) => setPromoForm({ ...promoForm, sticker: e.target.value })} className="bg-neutral-800 rounded-lg p-2 text-sm border border-neutral-700 text-white">{STICKER_OPTS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>
              </div>
            </div>
