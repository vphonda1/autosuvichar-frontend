...
export default function App() {
  ...
  const [promoForm, setPromoForm] = useState({ model: "", price: "", downPayment: "", cashback: "", features: "", file: null, bg: "light", vehicle: "", cutout: true, aiPrompt: "", offers: [], sticker: "" });
  ...
  async function doGenerate() { setErr(""); setBusy(true); try { await api("/api/generate", { method: "POST", body: JSON.stringify({ brand: brandId, type, offer: cOffers.join(","), sticker: cSticker }) }); await load(); } catch (e) { setErr(e.message); } setBusy(false); }
  ...
  async function submitPromo() {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error || "upload failed");
      setPromoForm({ model: "", price: "", downPayment: "", cashback: "", features: "", file: null, bg: "light", vehicle: "", cutout: true, aiPrompt: "", offers: [], sticker: "" });
      setTab("content");
    } catch (e) { setErr(e.message); } setBusy(false);
  }
        ...
        {tab === "content" && (<>
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-4 mb-5">
            ...
            <div className="mb-3">
              ...
              <label className="text-xs text-neutral-400 block mt-2">Sticker design
        ...
        {tab === "promo" && (
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-4 mb-5 space-y-3">
            ...
            <div>
              ...
              <label className="text-xs text-neutral-400 block mt-2">Sticker design
                <select value={promoForm.sticker} onChange={(e) => setPromoForm({ ...promoForm, sticker: e.target.value })} className="w-full bg-neutral-800 rounded-lg p-2 text-sm border border-neutral-700 text-white mt-1">{STICKER_OPTS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></label>
            </div>
