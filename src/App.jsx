...
export default function App() {
  ...
  const [promoForm, setPromoForm] = useState({ model: "", price: "", downPayment: "", cashback: "", features: "", file: null, bg: "light", vehicle: "", cutout: true, aiPrompt: "", offers: [], stickers: [], decor: [] });
  ...
  async function doGenerate() { setErr(""); setBusy(true); try { await api("/api/generate", { method: "POST", body: JSON.stringify({ brand: brandId, type, offer: cOffers.join(","), sticker: cStickers.join(","), decor: cDecor.join(",") }) }); await load(); } catch (e) { setErr(e.message); } setBusy(false); }
  ...
  async function submitPromo() {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error || "upload failed");
      setPromoForm({ model: "", price: "", downPayment: "", cashback: "", features: "", file: null, bg: "light", vehicle: "", cutout: true, aiPrompt: "", offers: [], stickers: [], decor: [] });
      setTab("content");
    } catch (e) { setErr(e.message); } setBusy(false);
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {tab === "promo" && (
          <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-4 mb-5 space-y-3">
            ...
            <button type="button" onClick={() => window.open("/vphonda-editor.html", "_blank")} className="w-full rounded-xl py-3 font-semibold text-white border border-neutral-600 bg-neutral-800">✏️ खुद design/edit करें (Poster Editor खोलें)</button>
