...
const CONTENT_BG = [["showroom", "🏬 शोरूम"], ["studio", "📸 स्टूडियो"], ["diwali", "🪔 दिवाली"], ["holi", "🎨 होली"], ["navratri", "🌼 नवरात्रि"], ["city", "🌆 शहर"], ["sport", "🔴 लाल स्पोर्टी"], ["blue", "🔵 नीला"]];
...
export default function App() {
  ...
  const [setBrandId, setSetBrandId] = useState("vp_honda"); const [savedNote, setSavedNote] = useState(""); const [health, setHealth] = useState(null);
  ...
  async function doGenerate() { setErr(""); setBusy(true); try { await api("/api/generate", { method: "POST", body: JSON.stringify({ brand: brandId, type, offer: cOffers.join(","), sticker: cStickers.join(","), decor: cDecor.join(","), bg: cBg, tags: cTags.join(" "), festival: type === "festival" ? cFestival : undefined }) }); await load(); } catch (e) { setErr(e.message); } setBusy(false); }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {tab === "settings" && isAdmin && (<>
          ...
          <label className="text-xs text-neutral-400 block mb-2">Brand चुनें
            <select value={setBrandId} onChange={(e) => setSetBrandId(e.target.value)} className="w-full bg-neutral-800 rounded-lg p-2 text-sm border border-neutral-700 mt-1 text-white">
          ...
          {brands[setBrandId] && <SettingCard key={setBrandId} id={setBrandId} b={brands[setBrandId]} cur={settings[setBrandId] || {}} onSave={saveSetting} onYT={connectYouTube} busy={busy} />}
...
const SentRow = ({ s, kind, onResend, onDel, busy }) => (
  <div className="rounded-xl bg-neutral-900/60 border border-neutral-800 px-3 py-2.5">
    <div className="flex gap-2">
      {s.images && s.images.square && <img src={media(s.images.square)} alt="" className="w-16 h-16 rounded-lg object-cover border border-neutral-700 flex-shrink-0" />}
      <div className="flex-1 min-w-0">
        ...
        <div className="flex gap-2 mt-1.5">
          {s.images && s.images.square && <a href={media(s.images.square)} target="_blank" rel="noreferrer" className="text-[11px] rounded-lg px-2 py-1 border border-neutral-700 text-neutral-300">👁 खोलें</a>}
          <button type="button" disabled={busy} onClick={() => onResend(kind, s._id)} className="text-[11px] rounded-lg px-2 py-1 font-semibold text-black" style={{ background: "#ffd400" }}>📤 दोबारा भेजें</button>
          <button type="button" disabled={busy} onClick={() => onDel(kind, s._id)} className="text-[11px] rounded-lg px-2 py-1 border border-red-700 text-red-400">🗑 delete</button>
        </div>
      </div>
    </div>
  </div>
