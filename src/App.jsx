    ...
    <div style={{ fontFamily: "system-ui" }} className="min-h-screen bg-neutral-950 text-neutral-100 px-4 py-5">
      <div className="max-w-3xl mx-auto">
        ...
        {tab === "content" && (<>
          ...
          <div className="space-y-2 pb-10">{sent.length === 0 && <Empty>अभी कुछ नहीं भेजा गया।</Empty>}{sent.map((s) => <SentRow key={s._id} s={s} kind="content" onResend={resend} busy={busy} />)}</div>
        </>)}
        ...
        {tab === "delivery" && (<>
          ...
          <div className="space-y-2 pb-10">{sentDeliv.length === 0 && <Empty>अभी कोई delivery नहीं भेजी।</Empty>}{sentDeliv.map((s) => <SentRow key={s._id} s={s} kind="delivery" onResend={resend} busy={busy} />)}</div>
        </>)}
        {tab === "leads" && (<>
...
const SentRow = ({ s, kind, onResend, busy }) => (
