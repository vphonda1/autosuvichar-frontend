...
export default function App() {
  ...
  const [resendBusy, setResendBusy] = useState({}); // per-item busy
  ...
  async function resend(kind, id) {
    ...
    try {
      ...
      setResendNote(ok ? `✅ भेजा: ${ok}` + (fail ? ` | ❌ ${fail}` : "") : `❌ सब fail: ${fail || "unknown"}`);
    ...
    <div style={{ fontFamily: "system-ui" }} className="min-h-screen bg-neutral-950 text-neutral-100 px-4 py-5">
      <div className="max-w-3xl mx-auto">
        ...
        {tab === "content" && (<>
          ...
          {resendNote && <div className={`rounded-xl px-4 py-3 text-sm font-semibold mb-2 ${resendNote.startsWith("✅") ? "bg-emerald-900/60 text-emerald-300 border border-emerald-700" : "bg-red-900/60 text-red-300 border border-red-700"}`}>{resendNote}</div>}
