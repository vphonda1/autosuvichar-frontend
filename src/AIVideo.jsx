...
export default function AIVideo({ apiBase, token, brandId, onSent }) {
  ...
  async function makeCaption() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/delivery/ai-analyze", {
        method: "POST",
  ...
  async function makeVideo() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/video/slideshow", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
        body: JSON.stringify({
          brand: brandId,
  ...
  // ── Poll status ──────────────────────────────────────────────
  //  ⚠️ पहले हर 2 सेकंड, चाहे फ़ोन जेब में हो. अब tab पीछे जाते ही रुक जाता है
  //     और वापस आते ही तुरन्त देख लेता है. 3 मिनट की हद पहले जैसी ही है.
  useEffect(() => {
    if (!jobId) return;
    let tries = 0;
    let stopped = false;

    const stop = () => { stopped = true; clearTimeout(pollRef.current); pollRef.current = null; };

    const tick = async () => {
      if (stopped) return;

      // पर्दा पीछे है → इस बार मत पूछो
      if (document.hidden) {
      ...
      try {
        const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/video/status/${jobId}`, { headers: { Authorization: "Bearer " + token } });
        const d = await r.json();
        setStatus(d);
        if (d.status === "done") {
          stop(); setBusy(false);
      ...
      // 2s से शुरू, धीरे-धीरे 8s तक
      pollRef.current = setTimeout(tick, Math.min(2000 + tries * 150, 8000));
    };
    ...
    <div className="space-y-3 pb-10">
      ...
      {status?.status === "done" && status.url && (
        <div className="rounded-2xl bg-neutral-900 border border-emerald-700 p-3 space-y-2">
          ...
          <video src={apiBase.replace(/\/+$/, "") + status.url} controls playsInline className="w-full rounded-xl bg-black" />
          <div className="grid grid-cols-2 gap-2">
            <a href={apiBase.replace(/\/+$/, "") + status.url} download="delivery-video.mp4"
