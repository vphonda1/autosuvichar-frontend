...
const STATIC_SUG = {
  ...
  suchna: ["आज दुकान बंद रहेगी", "कल से नया स्टॉक आ गया", "छुट्टी की सूचना", "समय बदल गया है"],
        ...
        if (capId === "vigyapan") {
          try {
            ...
            if (!cancelled) {
              ...
              setFestList((up.length ? up : (fd.festivals || [])).slice(0, 12).map((f) => f.name));
            }
          } catch (_) { setFestList([]); }
        } else setFestList([]);
        try {
    ...
    try {
      ...
      const e = await fetch(`${apiBase.replace(/\/+$/, "")}/api/command/execute`, {
        ...
        body: JSON.stringify({
          ...
          festival: adFestival || undefined,   // विज्ञापन के साथ जोड़ा हुआ त्यौहार
        }),
      });
      ...
      <div className="rounded-2xl bg-neutral-900 border-2 p-3 space-y-2.5" style={{ borderColor: accent + "66" }}>
        ...
        {capId === "vigyapan" && festList.length > 0 && (
          <div>
            ...
            <select value={adFestival} onChange={(e) => { setAdFestival(e.target.value); vib(15); }}
              className="w-full bg-neutral-800 border border-neutral-700 rounded-xl p-2.5 text-sm text-white outline-none">
