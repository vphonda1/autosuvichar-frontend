...
function SmartBar({ apiBase, token, brandId, accent, onOpen, onDone }) {
  ...
  async function samjho() {
    ...
    setStep("समझ रहे हैं…");
    try {
      // ⚠️ पहले हर बार server से पूछना पड़ता था — धीमा, और उसकी सूची अलग थी।
      //    अब वही सूची यहीं है, इसलिए पहचान तुरन्त होती है।
      const local = matchCap(txt);
      if (local && !local.ai) {
        // हाथ वाला काम — पन्ना खोलो, भरने का डेटा server से माँग लो
        ...
        } catch (_) { /* भरा हुआ न मिले तो भी पन्ना खुले */ }
        setStep(""); setMsg(`${local.icon} ${local.label} — देख लीजिए, फिर भेज दीजिए`);
        onOpen(local.id, draft, warn);
        setTxt(""); setBusy(false);
        return;
      }

      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/command/route`, {
        method: "POST", headers: auth,
        body: JSON.stringify({ command: txt.trim(), brand: brandId }),
      });
...
export default function Studio({ apiBase, token, brandId, accent, isAdmin, onChange }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ⚠️ सबसे ऊपर — एक बात कहिए, 8 चीज़ें एक साथ बन जाएँ।
          पहले हर चीज़ के लिए अलग पन्ना खोलना पड़ता था। */}
      <div>
        <Title>एक बात कहिए — सब बन जाएगा</Title>
        <CommandCenter apiBase={apiBase} token={token} brandId={brandId}
          accent={accent} onDone={onChange} />
      </div>

      {/* ⚠️ एक poster पसंद न आए तो दोबारा बनवाना पड़ता था — अब तीनों एक
          साथ, तीन अलग लहजों में। एक चुनिए, बाक़ी दो अपने आप हट जाएँ। */}
      <div>
        <Title>या तीन में से चुनना है?</Title>
        <ThreeChoice apiBase={apiBase} token={token} brandId={brandId}
          accent={accent} onDone={onChange} />
      </div>

      <div>
        <Title>या कोई एक चीज़ बनानी है?</Title>
