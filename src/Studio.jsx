...
const SUCHNA_FORMS = [
  { id: "samay", label: "🕒 समय बदल गया",
    ...
    make: (f) => `आवश्यक सूचना: ${f.date ? f.date + " से " : ""}शोरूम का समय बदल गया है। अब ${f.from || "सुबह"} से ${f.to || "शाम"} तक खुला रहेगा। आपकी सेवा में हमेशा तत्पर — अब और भी ज़्यादा समय, आपकी सुविधा के लिए!` },
  { id: "camp", label: "🔧 फ्री सर्विस कैंप",
    fields: [{ k: "place", ph: "गाँव/जगह का नाम" }, { k: "date", ph: "तारीख़ — 22 सितंबर" }, { k: "time", ph: "सुबह 10 से शाम 5" }],
    make: (f) => `आवश्यक सूचना: फ्री सर्विस कैंप — ${f.place || "आपके क्षेत्र"} में${f.date ? ", " + f.date + " को" : ""}${f.time ? ", " + f.time + " तक" : ""}। अपनी गाड़ी की फ्री जाँच करवाइए — ऐसे मौके बार-बार नहीं आते! सभी ग्राहक सादर आमंत्रित हैं। 🔧` },
  { id: "sthan", label: "📍 शोरूम का स्थान बदला",
    fields: [{ k: "old", ph: "पुराना पता" }, { k: "newp", ph: "नया पता" }, { k: "date", ph: "कब से — 1 अक्टूबर" }],
    make: (f) => `आवश्यक सूचना: हमारा शोरूम ${f.old ? f.old + " से " : ""}${f.newp || "नए पते"} पर स्थानांतरित हो गया है${f.date ? ", " + f.date + " से" : ""}। नए, बड़े और बेहतर शोरूम में आपका हार्दिक स्वागत है — एक बार ज़रूर पधारिए! 🙏` },
  { id: "chutti", label: "🏠 छुट्टी की सूचना",
    fields: [{ k: "date", ph: "तारीख़ — 25 सितंबर" }, { k: "reason", ph: "कारण (वैकल्पिक)" }, { k: "open", ph: "दोबारा कब खुलेगा" }],
    make: (f) => `आवश्यक सूचना: ${f.date || "कल"} को शोरूम बंद रहेगा${f.reason ? " (" + f.reason + ")" : ""}। ${f.open ? f.open + " से सामान्य रूप से खुलेगा।" : ""} असुविधा के लिए खेद है — आपके सहयोग के लिए धन्यवाद! 🙏` },
  { id: "stock", label: "🏍️ नया स्टॉक आया",
    fields: [{ k: "what", ph: "कौन-सी गाड़ी/मॉडल" }, { k: "when", ph: "कब से उपलब्ध" }],
    make: (f) => `आवश्यक सूचना: ${f.what || "नया स्टॉक"} अब शोरूम में उपलब्ध है${f.when ? ", " + f.when + " से" : ""}। आइए, देखिए और मुफ़्त टेस्ट राइड लीजिए — इस मौके का पूरा फ़ायदा उठाइए! 🏍️` },
  { id: "staff", label: "👨‍🔧 नया स्टाफ आया",
    fields: [{ k: "who", ph: "पद — जैसे अनुभवी मैकेनिक" }, { k: "when", ph: "कब से" }],
    make: (f) => `आवश्यक सूचना: हमारे शोरूम में ${f.who || "नया स्टाफ"} आ गया है${f.when ? ", " + f.when + " से" : ""}। अब आपको और भी तेज़, भरोसेमंद सेवा मिलेगी — आपका भरोसा ही हमारी पहचान! ⭐` },
...
function CreatePanel({ apiBase, token, brandId, accent, onOpen, onDone }) {
  ...
  const [festBg, setFestBg] = useState("gehra");      // त्यौहार poster का background
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (capId === "festival") {
        //  ⚠️ पहले सिर्फ़ 5 आने वाले त्यौहार दिखते थे — गणेश चतुर्थी जैसा
        //     अभी चल रहा त्यौहार तारीख़ निकलते ही ग़ायब हो जाता। अब बड़े
        //     त्यौहार हमेशा सबसे ऊपर, बीच के छोटे-मोटे दिन भी, और अंत में
        //     "ख़ुद लिखें" — ताकि कोई भी मौक़ा छूटे नहीं।
        const BADE = ["गणेश चतुर्थी", "नवरात्रि", "दशहरा", "धनतेरस", "दिवाली", "भाई दूज", "करवा चौथ", "छठ पूजा", "होली", "रक्षाबंधन", "जन्माष्टमी", "मकर संक्रांति"];
        const CHOTE = ["नया साल", "स्वतंत्रता दिवस", "गणतंत्र दिवस", "रविवार स्पेशल", "दुकान की सालगिरह", "महीने का आख़िरी ऑफ़र"];
        try {
          const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/festivals`);
          const d = await r.json();
          if (cancelled) return;
          const today = new Date().toISOString().slice(0, 10);
          const up = (d.festivals || []).filter((f) => f.date >= today).slice(0, 6).map((f) => f.name);
          const all = [...new Set([...up, ...BADE, ...CHOTE])];
          setSugg([...all.map((n) => `${n} की बधाई`), "✏️ ख़ुद लिखें — कोई और मौक़ा"]);
        } catch (_) {
          setSugg([...[...BADE, ...CHOTE].map((n) => `${n} की बधाई`), "✏️ ख़ुद लिखें — कोई और मौक़ा"]);
        }
        return;
      }
    ...
    try {
      ...
      const e = await fetch(`${apiBase.replace(/\/+$/, "")}/api/command/execute`, {
        ...
        body: JSON.stringify({
          ...
          //  ⚠️ बड़ी गड़बड़ी पकड़ी — पहले सिर्फ़ `intent.custom_text` (AI का
          //     निकाला हुआ) भेजा जाता था, और आपका ख़ुद भरा हुआ पूरा text
          //     (txt) फेंक दिया जाता। AI टूटी हो या custom_text न लौटाए,
          //     तो content बिल्कुल ख़ाली चला जाता — "post बन गई" तो लिखा
          //     आता, पर poster में कुछ होता ही नहीं (यही आपके साथ हुआ)।
          //     सूचना में आपका भरा हुआ text ही असली सूचना है, इसलिए वही
          //     सीधे भेजते हैं। विज्ञापन/सुविचार में ऐसा नहीं करते — वहाँ
          //     आपका text सिर्फ़ आदेश होता है ("Shine का ऑफर बनाओ"), उसे
          //     caption बना देना ग़लत होगा; वहाँ AI ही लिखे, यही सही है।
          custom_text: forcedAutoType === "suchna" ? txt.trim() : (intent.custom_text || ""),
          schedule: intent.schedule || { when: "now" },
          //  ⚠️ असली गड़बड़ी यहीं थी — "त्यौहार" साँचा चुनने पर adFestival
          //     ख़ाली रहता (वो सिर्फ़ विज्ञापन वाले dropdown से भरता है),
          //     इसलिए त्यौहार का नाम backend तक पहुँचता ही नहीं था। नाम न
          //     मिलने से न title में त्यौहार आता, न उसकी तस्वीर ढूँढी जाती।
          //     अब त्यौहार वाले साँचे में आपका लिखा/चुना text ही भेजते हैं।
          festBg,
          festival: (forcedAutoType === "festival" && txt.trim())
            ? txt.trim()
            : ((adFestival && adFestival.trim()) ? adFestival.trim() : undefined),
        }),
      });
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl bg-neutral-900 border-2 p-3 space-y-2.5" style={{ borderColor: accent + "66" }}>
        ...
        {/* ⚠️ नया — त्यौहार poster का background, आपकी पसंद से — हर बार
            बताने की ज़रूरत नहीं, यहीं से चुन लीजिए */}
        {capId === "festival" && (
          <div>
            <label className="text-[11px] text-neutral-500 block mb-1">Poster का background</label>
            <select value={festBg} onChange={(e) => { setFestBg(e.target.value); vib(15); }}
              className="w-full bg-neutral-800 border border-neutral-700 rounded-xl p-2.5 text-sm text-white outline-none">
              <option value="gehra">🎨 गहरा (brand रंग) — वॉटरमार्क के साथ</option>
              <option value="safed">⬜ सफ़ेद/हल्का — वॉटरमार्क के साथ</option>
            </select>
          </div>
        )}
        ...
        {capId && sugg.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {sugg.map((x, i) => (
                <button key={i} type="button" onClick={() => { vib(15); setTxt(x.startsWith("\u270F") ? "" : x); }}
