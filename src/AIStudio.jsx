...
export default function AIStudio({ apiBase, token, brandId, onSent }) {
  ...
  async function runAdapt() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/adapt", {
  ...
  async function runVariants() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/variants/caption", {
        method: "POST", headers: H,
  ...
  async function chooseVariant(i) {
    ...
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/variants/${vSet.setId}/choose`, {
