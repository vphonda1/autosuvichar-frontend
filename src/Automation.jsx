...
export default function Automation({ apiBase, token, brandId }) {
  ...
  async function load() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/automation/${scope}`, { headers: { Authorization: "Bearer " + token } });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "Error");
      setS({ ...d.settings, autoTypes: d.settings.autoTypes || [], autoChannels: d.settings.autoChannels || ["fb", "ig"] });
      setUsage(d.usage);
      const hr = await fetch(`${apiBase.replace(/\/+$/, "")}/api/automation/${scope}/usage-history`, { headers: { Authorization: "Bearer " + token } });
  ...
  async function save() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/automation/${scope}`, {
        method: "PUT",
