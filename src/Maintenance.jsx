...
export default function Maintenance({ apiBase, token, brandId }) {
  ...
  async function get(url, setter) {
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + url, { headers: H });
  ...
  async function runTrim() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/storage/trim", { method: "POST", headers: H });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Error");
  ...
  async function runCleanup() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/storage/cleanup", { method: "POST", headers: H });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Error");
  ...
  async function runRetry() {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/retry-queue/run", { method: "POST", headers: H });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error || "Error");
