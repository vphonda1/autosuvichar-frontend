...
export default function Announcer({ apiBase, token, brandId, onSent, draft }) {
  // दो तरीक़े:
  ...
  const full = (u) => (u?.startsWith("http") ? u : apiBase.replace(/\/+$/, "") + u);
  const set = (k, val) => setV((p) => ({ ...p, [k]: val }));

  useEffect(() => {
    fetch(`${apiBase.replace(/\/+$/, "")}/api/announce/options`, { headers: auth })
      .then((r) => r.json()).then(setOpt).catch((e) => setErr(e.message));
    load();
    // eslint-disable-next-line
  }, [brandId]);

  // बोलकर बनवाया हुआ — text और अंदाज़ पहले से भरा मिलेगा
  useEffect(() => {
    if (!draft) return;
  ...
  function load() {
    fetch(`${apiBase.replace(/\/+$/, "")}/api/announce?brand=${brandId}`, { headers: auth })
  ...
  function sunao(id) {
    ...
    try {
      ...
      const a = new Audio(`${apiBase.replace(/\/+$/, "")}/api/announce/sfx/${id}`);
      sfxRef.current = a;
      a.play().catch(() => {});
    } catch (_) {}
  }
  ...
  async function banao() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/announce/build`, {
        method: "POST",
  ...
  async function jaancho() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/announce/script/parse`, {
  ...
  async function scriptBanao() {
    ...
    try {
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/announce/script/build`, {
        method: "POST", headers: { "Content-Type": "application/json", ...auth },
    ...
    <div className="space-y-3 pb-10">
          ...
          {scriptOut && (
            <div className="rounded-2xl border-2 p-3 space-y-2.5" style={{ borderColor: "#F97316", background: "#F9731610" }}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <a href={`${apiBase.replace(/\/+$/, "")}/announce-download/${scriptOut.id}`} onClick={() => vib(30)}
      ...
      {made && (
        <div className="rounded-2xl border-2 p-3 space-y-2.5" style={{ borderColor: "#F97316", background: "#F9731610" }}>
          ...
          <div className="grid grid-cols-2 gap-2">
            <a href={`${apiBase.replace(/\/+$/, "")}/announce-download/${made.id}`}
              onClick={() => vib(30)}
              className="rounded-lg py-2.5 text-center text-xs font-bold text-white" style={{ background: "#F97316" }}>
              ⬇️ Download
            </a>
            ...
            {list.slice(0, 8).map((a) => (
              <div key={a._id} className="rounded-xl bg-neutral-900 border border-neutral-800 p-2.5">
                ...
                <div className="flex gap-2 mt-1.5">
                  <a href={`${apiBase.replace(/\/+$/, "")}/announce-download/${a._id}`}
                  ...
                  <button type="button" onClick={async () => {
                    ...
                    await fetch(`${apiBase.replace(/\/+$/, "")}/api/announce/${a._id}`, { method: "DELETE", headers: auth });
                    load();
