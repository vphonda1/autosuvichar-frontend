...
export default function GrowthCalendar({ apiBase, token, brandId, accent = "#E4002B", onChange }) {
  ...
  async function load() {
    ...
    try {
      const u = `${apiBase.replace(/\/+$/, "")}/api/calendar?brand=${brandId}&from=${range.from.toISOString()}&to=${range.to.toISOString()}`;
  ...
  async function schedulePost(contentId, dayKey, hour) {
    ...
    try {
      ...
      const r = await fetch(`${apiBase.replace(/\/+$/, "")}/api/schedule`, {
  ...
  async function cancelSchedule(id) {
    ...
    try {
      await fetch(`${apiBase.replace(/\/+$/, "")}/api/schedule/${id}`, {
            ...
            <div className="space-y-2 mb-3">
              {(byDay[openDay] || []).map((e) => (
                <div key={e.id} className="flex gap-2.5 items-start rounded-xl bg-neutral-950 border border-neutral-800 p-2">
                  {e.img
                    ? <img src={e.img.startsWith("/") ? apiBase.replace(/\/+$/, "") + e.img : e.img} alt="" className="w-12 h-12 rounded-lg object-cover flex-shrink-0" />
              ...
              <div className="space-y-2">
                {(data?.pending || []).map((p) => (
                  <button key={p.id} disabled={saving}
                    ...
                    {p.img
                      ? <img src={p.img.startsWith("/") ? apiBase.replace(/\/+$/, "") + p.img : p.img} alt="" className="w-14 h-14 rounded-lg object-cover flex-shrink-0" />
