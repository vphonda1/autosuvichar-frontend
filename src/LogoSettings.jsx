...
export default function LogoSettings({ apiBase, token }) {
  ...
  async function load() {
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/logos", { headers: H });
  ...
  async function assign(brand, key, value) {
    ...
    try {
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/logos", {
  ...
  async function upload(e) {
    ...
    try {
      ...
      fd.append("logo", f);
      const r = await fetch(apiBase.replace(/\/+$/, "") + "/api/logos/upload", {
            ...
            {files.map((f) => (
              <div key={f} className="text-center">
                <div className="rounded-xl bg-neutral-800 p-2 h-20 flex items-center justify-center">
                  <img src={`${apiBase.replace(/\/+$/, "")}/logos/${f}`} alt={f} className="max-h-full max-w-full object-contain" />
                </div>
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="text-center flex-1">
                  <div className="h-14 flex items-center justify-center">
                    {cfg.ownerLogo
                      ? <img src={`${apiBase.replace(/\/+$/, "")}/logos/${cfg.ownerLogo}`} alt="" className="max-h-full object-contain" />
                ...
                <div className="text-center flex-1">
                  <div className="h-14 flex items-center justify-center">
                    {cfg.companyLogo
                      ? <img src={`${apiBase.replace(/\/+$/, "")}/logos/${cfg.companyLogo}`} alt="" className="max-h-full object-contain" />
                      : <span className="text-xs text-neutral-600">— खाली —</span>}
                  </div>
