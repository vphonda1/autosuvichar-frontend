...
export default function HiringEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  // ── बोलकर बनवाया हुआ draft — असली कंपनी नाम, पद ──────────
  useEffect(() => {
    if (!draft) return;
    if (draft.shopName) { setCompanyName(draft.shopName); setTouched(true); }
    if (draft.phone) setPhone(draft.phone);
    if (draft.address) setAddress(draft.address);
    if (draft.roles) {
      // ⚠️ पहली "row" वाली पंक्ति में असली पद डाल देते हैं — बाक़ी पंक्तियाँ
      //    जस की तस रहती हैं क्योंकि "roles" एक ही सादा वाक्य है, अलग-अलग
      //    गिनती के साथ बँटा हुआ structured data नहीं।
      setElems((prev) => {
        let done = false;
        return prev.map((el) => {
          if (!done && el.type === "row") { done = true; return { ...el, text: draft.roles }; }
          return el;
        });
      });
      setCaption(`🔥 We Are Hiring! ${draft.shopName || ""} में ${draft.roles}। Walk-in Interview के लिए अभी संपर्क करें। #Hiring`);
    }
  }, [draft]);
  ...
  const render = useCallback(() => {
    ...
    if (qrImg) {
      // ✅ cache से — QR खिंचकर टेढ़ा न हो, वरना scan नहीं होता
      const im = imgCache.get(qrImg);
      if (im) drawFit(ctx, im, qrPos.x, qrPos.y, qrPos.w, qrPos.h);
    } else {
  ...
  }, [bg, qrImg, qrPos, elems, selId, companyName, phone, address, brandId, apiBase, logoTick, ownerTick, imgCache.tick]);

  function draw3DRect(ctx, x, y, w, h, bs, text, fs) {
    const r = Math.min(14, h * .25);
    ctx.fillStyle = bs.sh; ctx.beginPath(); roundRect(ctx, x+4, y+6, w, h, r); ctx.fill();
    const g = ctx.createLinearGradient(x, y, x, y+h);
    g.addColorStop(0, lighten(bs.bg, 28)); g.addColorStop(1, bs.bg);
    ctx.fillStyle = g; ctx.beginPath(); roundRect(ctx, x, y, w, h, r); ctx.fill();
    ctx.fillStyle = "rgba(255,255,255,.18)"; ctx.beginPath(); roundRect(ctx, x+3, y+3, w-6, h*.42, [r,r,0,0]); ctx.fill();
    ctx.strokeStyle = bs.br; ctx.lineWidth = 2.5; ctx.beginPath(); roundRect(ctx, x, y, w, h, r); ctx.stroke();
