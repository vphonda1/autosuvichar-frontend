...
// ⚠️ पहले "draft" prop का यहाँ ज़िक्र तक नहीं था — बोलकर "Activa का मेगा
//    ऑफर" कहने पर command-router.js सही गाड़ी/क़ीमत खोज तो लेता था, पर वो
//    data यहाँ तक आते ही चुपचाप फेंक दिया जाता था। पन्ना अपने पुराने
//    placeholder ("₹10,000", "SP 160" आदि) के साथ ही खुलता था।
export default function MegaOfferEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  // ── बोलकर बनवाया हुआ draft — असली गाड़ी, असली क़ीमत ──────────
  //  ⚠️ elems के शुरुआती (index-based) placeholder ₹10,000/₹5,000/₹3,000
  //     जैसे मनगढ़ंत अंक हैं — draft में असली cashback/exchangeBonus/roi
  //     मिले तो उन्हीं इंडेक्स पर सही अंक बिठा देते हैं। बाक़ी elems
  //     (headline sub-line, corporate discount) जस के तस रहते हैं क्योंकि
  //     उनका कोई सीधा database खाना नहीं है।
  useEffect(() => {
    if (!draft) return;
    if (draft.headline) setHeadline(draft.headline);
    if (draft.bikeImg) setBikeImg(draft.bikeImg);
    if (draft.shopName) { setDealerName(draft.shopName); setTouchedDealer(true); }
    if (draft.shopSub) setDealerSub(draft.shopSub);
    if (draft.phone) setPhone(draft.phone);

    setElems((prev) => prev.map((el, i) => {
      if (i === 1 && (draft.price || draft.downPayment)) {
        const amt = draft.downPayment || draft.price;
        return { ...el, text: `${amt} में\nमहाबचत` };
      }
      if (i === 2 && draft.cashback) return { ...el, text: `💰 ${draft.cashback} कैशबैक` };
      if (i === 3 && draft.exchangeBonus) return { ...el, text: `🔄 ${draft.exchangeBonus} एक्सचेंज बोनस` };
      if (i === 5 && draft.roi) return { ...el, text: `सिर्फ\n${draft.roi}\nब्याज दर` };
      return el;
    }));

    if (draft.vehicleName) {
      setCaption(
        `🔥 ${draft.vehicleName} पर धमाकेदार ऑफर! अभी visit करें।\n` +
  ...
  const render = useCallback(() => {
    ...
    if (bikeImg) {
      // ✅ cache से — तस्वीर एक बार load, अनुपात भी सही
      const img = imgCache.get(bikeImg);
      if (img) drawFit(ctx, img, 0, H*0.28, W*0.52, H*0.56);
    } else {
      ctx.save();
      ctx.globalAlpha = 0.12;
      ctx.fillStyle = "#fff";
      roundRect(ctx, 20, H*0.30, W*0.5, H*0.52, 16);
      ctx.fill();
      ctx.restore();
      ctx.fillStyle = "#fff";
      ctx.globalAlpha = 0.4;
    ...
    elems.forEach(el => {
      ...
      if (el.type === "circle") {
        ...
        ctx.beginPath();
        roundRect(ctx, x+3, y+3, w-6, h*0.45, [r,r,0,0]);
        ctx.fill();
        // Border
        ctx.strokeStyle = bs.border;
        ctx.lineWidth = 2.5;
  ...
  }, [bg, bikeImg, headline, hlSize, elems, selId, showAddr, addrStyle, dealerName, dealerSub, phone, brandId, apiBase, logoTick, ownerTick, imgCache.tick]);
  ...
  async function sendToReview() {
    ...
    try {
      ...
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/mega-offer/submit", {
        method:"POST",
    ...
    <div className="space-y-3 pb-10">
      ...
      <div className="sticky top-0 z-30 bg-neutral-950 pb-2 pt-1">
        <div className="rounded-2xl overflow-hidden border border-neutral-700 bg-black">
          ...
          {/* ✅ अपनी गाड़ी चुनें — photo और दाम अपने आप */}
          <div className="px-3 pt-3">
            <VehiclePicker apiBase={apiBase} token={token} brandId={brandId}
              onPick={(v) => {
                if (v.photo) setBikeImg(v.photo);
                if (v.fullName) setHeadline(v.fullName.replace(/ /, "\n"));
              }} />
          </div>
