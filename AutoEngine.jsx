...
export default function AutoEngine({ apiBase, token, brandId, onSent }) {
  ...
  async function api(url, opts) {
    const r = await fetch(apiBase.replace(/\/+$/, "") + url, { headers: H, ...opts });
  ...
  // ── batch polling ─────────────────────────────────────────
  //  ⚠️ पहले यहाँ setInterval 3 सेकंड का था और उसकी कोई हद नहीं थी। अगर कोई
  //     batch server पर अटक जाता (restart हो गया, job मर गया), तो यह tab
  //     हमेशा-हमेशा हर 3 सेकंड request भेजता रहता — रात भर, दुकान बन्द होने
  //     के बाद भी। Render की service इसी वजह से कभी सोती ही नहीं थी।
  //
  //  अब तीन बचाव:
  //    1. हद — 200 बार के बाद ख़ुद रुक जाता है
  //    2. tab पीछे गया तो पूछना बन्द, सामने आते ही एक बार तुरन्त
  //    3. शुरू में 3 सेकंड, फिर धीरे-धीरे 15 सेकंड तक — लम्बा काम हो तो
  //       कम request. पहले कुछ सेकंड तेज़ ही रहता है, इसलिए धीमा नहीं लगेगा।
  ...
  function startPolling(jobId) {
    ...
    const tick = async () => {
      if (pollRef.current === null && triesRef.current > 0) return;   // रोक दिया गया

      // tab पीछे है → इस बार कुछ मत पूछो, बस दोबारा आने का समय तय कर लो
      if (document.hidden) {
      ...
      if (triesRef.current > 200) {
        stopPolling();
        setBusy("");
        setNote("⚠️ बहुत समय लग रहा है — बाद में Review में देखें");
        return;
      }
      ...
      // 3s → 15s तक धीरे-धीरे
      const gap = Math.min(3000 + triesRef.current * 400, 15000);
      pollRef.current = setTimeout(tick, gap);
    };

    pollRef.current = setTimeout(tick, 0);
  }

  // पन्ना छोड़ते ही सब बन्द — कोई timer पीछे न रह जाए
  useEffect(() => () => stopPolling(), []);

  // जेब से फ़ोन निकालते ही एक बार तुरन्त देख लो
  useEffect(() => {
    const onVis = () => {
      if (!document.hidden && pollRef.current && jobRef.current) {
        api(`/api/batch/${jobRef.current}`).then(setJob).catch(() => {});
      }
    };
          ...
          <>
            ...
            {top.top.filter((x) => x.views || x.engagement).slice(0, 5).map((x, i) => (
              <div key={x._id} className="rounded-xl bg-neutral-900 border border-neutral-800 p-3 flex gap-3">
                {x.img && <img src={apiBase.replace(/\/+$/, "") + x.img} alt="" className="w-14 h-14 rounded-lg object-cover shrink-0" />}
