...
export default function AIDelivery({ apiBase, token, brandId, onSent, draft }) {
  ...
  const [customerMobile, setCustomerMobile] = useState("");   // भरा हो तो ग्राहक को भी जाएगी
  ...
  // ── बोलकर बनवाया हुआ draft — ग्राहक का नाम, गाड़ी ────────────
  //  ⚠️ ग्राहक की असली photo बोलकर नहीं दी जा सकती — वो आपको ख़ुद डालनी
  //     होगी। यह सिर्फ़ नाम/गाड़ी/caption भर देता है, फिर आप photo चुनिए।
  useEffect(() => {
    if (!draft) return;
    if (draft.customerName) setCustomerName(draft.customerName);
    if (draft.bikeName) setBikeName(draft.bikeName);
    if (draft.text) setCaption(draft.text);
  }, [draft]);
  ...
  async function pickBest() {
    ...
    try {
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/delivery/ai-pick-best", {
        method: "POST",
  ...
  async function generatePost() {
    ...
    vib(50); setAnalyzing(true); setNote("");
    try {
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/delivery/ai-analyze", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
  ...
  function renderCanvas(spec) {
    ...
    ctx.fillStyle = "#fff";
    ctx.beginPath(); roundRect(ctx, px - 10, py - 10, pw + 20, ph + 20, 20); ctx.fill();
    // ✅ पहले यहाँ हर render पर नई Image बनती थी और onload के भरोसे नीचे का
    //    हिस्सा खिंचता था। दो दिक़्क़तें थीं:
    //      1. तस्वीर न खुले तो drawBottom() कभी नहीं चलता — poster आधा रह जाता
    //      2. पुरानी तस्वीर का onload बाद में चलकर नई तस्वीर के ऊपर छप जाता
    //    अब cache से आती है — तैयार हो तो तुरन्त, वरना नीचे का हिस्सा फिर भी बनता है।
    const im = imgCache.get(selPhoto.dataUrl);
    if (im) {
      ctx.save();
      roundRect(ctx, px, py, pw, ph, 14); ctx.clip();
      ...
      // तस्वीर अभी आ रही है (या खुल नहीं पाई) — डिब्बा फिर भी दिखाओ
      ctx.save(); ctx.fillStyle = "#eee";
      roundRect(ctx, px, py, pw, ph, 14); ctx.fill(); ctx.restore();
    }
    drawBottom(ctx);
  ...
  // re-render on edits
  React.useEffect(() => { if (ai && selPhoto) renderCanvas(ai); }, [bg, headline, subLine, customerName, bikeName, selPhotoId, ai, brandId, logoTick, ownerTick, imgCache.tick]);
  ...
  return (
    <div className="space-y-3 pb-10">
      ...
      <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-4 space-y-2">
        ...
        <input value={customerMobile} onChange={e => setCustomerMobile(e.target.value)} inputMode="numeric" className={inp} placeholder="ग्राहक का WhatsApp (वैकल्पिक)" />
