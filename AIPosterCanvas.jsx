...
export default function AIPosterCanvas({ apiBase, token, brandId, spec, dealerName, dealerSub, phone, onSent, onBack }) {
  ...
  const render = useCallback(() => {
    ...
    if (bikeImg) {
      // ✅ cache से
      const im = imgCache.get(bikeImg);
      if (im) drawFit(ctx, im, bikePos.x, bikePos.y, bikePos.w, bikePos.h);
      if (selId === "__bike__") {
      ...
      ctx.beginPath(); roundRect(ctx, 20, H * .30, W * .5, H * .52, 16); ctx.fill(); ctx.restore();
    ...
    elems.forEach(el => {
      ...
      if (el.type === "circle") {
        ...
        ctx.fillStyle = bs.sh; ctx.beginPath(); roundRect(ctx, x + 5, y + 7, w, h, r); ctx.fill();
        const g2 = ctx.createLinearGradient(x, y, x, y + h);
        g2.addColorStop(0, lighten(bs.bg, 25)); g2.addColorStop(1, bs.bg);
        ctx.fillStyle = g2; ctx.beginPath(); roundRect(ctx, x, y, w, h, r); ctx.fill();
        ctx.fillStyle = "rgba(255,255,255,0.18)"; ctx.beginPath(); roundRect(ctx, x + 3, y + 3, w - 6, h * .45, [r, r, 0, 0]); ctx.fill();
  ...
  }, [bg, headline, hlSize, elems, selId, bikeImg, bikePos, dealerName, dealerSub, phone, brandId, apiBase, logoTick, ownerTick, imgCache.tick]);
        ...
        <div className="mt-2">
          <VehiclePicker apiBase={apiBase} token={token} brandId={brandId}
            onPick={(v) => {
              ...
              if (v.fullName) setHeadline(v.fullName);
            }} />
        </div>
