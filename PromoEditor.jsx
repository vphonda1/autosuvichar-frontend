...
// Sticker palette — सभी available shapes
const PALETTE = [
  "star", "heart", "flame", "gift", "sparkle", "check", "crown", "rupee",
  "party", "thumbsup", "sealOffer", "sealSale", "badgeNew",
  "loc", "phone", "whatsapp", "tick", "arrowR", "arrowD",
];
// Emoji/Sticker friendly names for UI display
const PALETTE_LABELS = {
  star: "⭐ स्टार", heart: "❤️ हार्ट", flame: "🔥 ज्वाला", gift: "🎁 गिफ्ट",
  sparkle: "✨ चमक", check: "✅ टिक", crown: "👑 ताज", rupee: "₹ रुपया",
  party: "🎉 पार्टी", thumbsup: "👍 थम्ब्स अप", sealOffer: "🔴 ऑफर सील",
  sealSale: "🟡 सेल सील", badgeNew: "🔵 NEW बैज", loc: "📍 लोकेशन",
  phone: "📞 फोन", whatsapp: "💬 WhatsApp", tick: "✔️ चेक", arrowR: "➡️ एरो", arrowD: "⬇️ नीचे",
};
...
function buildBadge(style, text) {
  ...
  switch (style) {
    ...
    // नए Honda dealer badge styles
    case "emiBox":      { const [l1,l2]=String(text||"").split("|").map(s=>s.trim()); return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#1565c0" stroke="#fff" stroke-width="3"/><text x="0" y="-6" text-anchor="middle" font-family="Arial,sans-serif" font-size="28" font-weight="700" fill="#fff">${esc(l1||"")}</text>${l2?`<text x="0" y="34" text-anchor="middle" font-family="Arial,sans-serif" font-size="46" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:""}`;  }
    case "roiBox":      { const [l1,l2]=String(text||"").split("|").map(s=>s.trim()); return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#141414" stroke="#ffd400" stroke-width="3"/><text x="0" y="-6" text-anchor="middle" font-family="Arial,sans-serif" font-size="28" font-weight="700" fill="#ffd400">${esc(l1||"")}</text>${l2?`<text x="0" y="34" text-anchor="middle" font-family="Arial,sans-serif" font-size="46" font-weight="900" fill="#fff">${esc(l2)}</text>`:""}`;  }
    case "cashbackBox": { const [l1,l2]=String(text||"").split("|").map(s=>s.trim()); return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#0ca678" stroke="#fff" stroke-width="3"/><text x="0" y="-6" text-anchor="middle" font-family="Arial,sans-serif" font-size="28" font-weight="700" fill="#fff">${esc(l1||"")}</text>${l2?`<text x="0" y="34" text-anchor="middle" font-family="Arial,sans-serif" font-size="44" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:""}`;  }
    case "megaOffer":   { const [l1,l2]=String(text||"").split("|").map(s=>s.trim()); return `<rect x="-185" y="-52" width="370" height="104" rx="8" fill="#E4002B"/><polygon points="-185,-52 -185,52 -145,0" fill="#7a0016"/><text x="10" y="-6" text-anchor="middle" font-family="Arial,sans-serif" font-size="30" font-weight="900" fill="#fff">${esc(l1||"")}</text>${l2?`<text x="10" y="36" text-anchor="middle" font-family="Arial,sans-serif" font-size="44" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:""}`;  }
    case "nagadChhut":  { const [l1,l2]=String(text||"").split("|").map(s=>s.trim()); return `<path d="M -170,-50 H 160 Q 180,-50 180,0 Q 180,50 160,50 H -170 L -185,0 Z" fill="#ffd400"/><text x="0" y="-6" text-anchor="middle" font-family="Arial,sans-serif" font-size="26" font-weight="700" fill="#141414">${esc(l1||"")}</text>${l2?`<text x="0" y="36" text-anchor="middle" font-family="Arial,sans-serif" font-size="44" font-weight="900" fill="#E4002B">${esc(l2)}</text>`:""}`;  }
    case "exchangeTag": { const [l1,l2]=String(text||"").split("|").map(s=>s.trim()); return `<rect x="-180" y="-50" width="360" height="100" rx="10" fill="#0ca678" stroke="#fff" stroke-width="3"/><text x="0" y="-6" text-anchor="middle" font-family="Arial,sans-serif" font-size="28" font-weight="700" fill="#fff">${esc(l1||"")}</text>${l2?`<text x="0" y="34" text-anchor="middle" font-family="Arial,sans-serif" font-size="40" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:""}`;  }
    case "loanFull":    { const [l1,l2]=String(text||"").split("|").map(s=>s.trim()); return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#7b1fa2" stroke="#fff" stroke-width="3"/><text x="0" y="-6" text-anchor="middle" font-family="Arial,sans-serif" font-size="28" font-weight="700" fill="#fff">${esc(l1||"")}</text>${l2?`<text x="0" y="34" text-anchor="middle" font-family="Arial,sans-serif" font-size="46" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:""}`;  }
    case "freeGiftTag": { const [l1,l2]=String(text||"").split("|").map(s=>s.trim()); return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#E4002B" stroke="#ffd400" stroke-width="4"/><text x="0" y="-6" text-anchor="middle" font-family="Arial,sans-serif" font-size="28" font-weight="700" fill="#ffd400">${esc(l1||"")}</text>${l2?`<text x="0" y="34" text-anchor="middle" font-family="Arial,sans-serif" font-size="38" font-weight="900" fill="#fff">${esc(l2)}</text>`:""}`;  }
    default: return "";
  }
}
const BADGES = [
  ...
  { style: "slantDual",   label: "⚡ दो-भाग (तिरछा)",    def: "डाउन|₹4999" },
  ...
  { style: "bookingOpen", label: "📋 बुकिंग ओपन",        def: "BOOKING OPEN" },
  { style: "emiBox",      label: "💳 EMI बॉक्स",         def: "Low EMI|₹1999/-" },
  { style: "roiBox",      label: "📊 ROI बॉक्स",         def: "Low ROI|@7.99%*" },
  { style: "cashbackBox", label: "💰 कैशबैक बॉक्स",     def: "Cashback|₹5000*" },
  { style: "megaOffer",   label: "🔥 महाबचत बैनर",      def: "महाबचत|₹10000 तक" },
  { style: "nagadChhut",  label: "💵 नगद छूट",           def: "नगद छूट|₹5000#" },
  { style: "exchangeTag", label: "🔄 एक्सचेंज बोनस",    def: "एक्सचेंज|₹2000*" },
  { style: "loanFull",    label: "🏦 Loan 100%",         def: "Loan upto|100%*" },
  { style: "freeGiftTag", label: "🎁 Free Gift",         def: "Free Gift|Worth ₹5999" },
];

// buildBadge में नए styles add करो
const _buildBadgeExtra = (style, text) => {
  const [l1, l2] = String(text || "").split("|").map((s) => s.trim());
  const F = "Arial,sans-serif";
  const T = (c, fs, dy, fill, wt=800) => `<text x="0" y="${dy}" text-anchor="middle" font-family="${F}" font-size="${fs}" font-weight="${wt}" fill="${fill}">${esc(l1||"")}</text>` + (l2 ? `<text x="0" y="${dy+fs*1.15}" text-anchor="middle" font-family="${F}" font-size="${fs*0.85}" font-weight="${wt}" fill="${fill=="#ffd400"?"#fff":fill}">${esc(l2)}</text>` : "");
  switch(style) {
    case "emiBox":      return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#1565c0" stroke="#fff" stroke-width="3"/>` + T("Mukta,sans-serif",28,-8,"#fff") + (l2?`<text x="0" y="34" text-anchor="middle" font-family="${F}" font-size="46" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:"");
    case "roiBox":      return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#141414" stroke="#ffd400" stroke-width="3"/>` + T("Mukta,sans-serif",28,-8,"#ffd400") + (l2?`<text x="0" y="34" text-anchor="middle" font-family="${F}" font-size="46" font-weight="900" fill="#fff">${esc(l2)}</text>`:"");
    case "cashbackBox": return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#0ca678" stroke="#fff" stroke-width="3"/>` + T("Mukta,sans-serif",28,-8,"#fff") + (l2?`<text x="0" y="34" text-anchor="middle" font-family="${F}" font-size="44" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:"");
    case "megaOffer":   return `<rect x="-185" y="-52" width="370" height="104" rx="8" fill="#E4002B"/><polygon points="-185,-52 -185,52 -145,0" fill="#7a0016"/>` + T("Mukta,sans-serif",30,-6,"#fff",900) + (l2?`<text x="10" y="36" text-anchor="middle" font-family="${F}" font-size="44" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:"");
    case "nagadChhut":  return `<path d="M -170,-50 H 160 Q 180,-50 180,0 Q 180,50 160,50 H -170 L -185,0 Z" fill="#ffd400"/><path d="-170,-50 -185,0 -170,50" fill="#e0a800"/>` + T("Mukta,sans-serif",26,-6,"#141414",700) + (l2?`<text x="0" y="36" text-anchor="middle" font-family="${F}" font-size="44" font-weight="900" fill="#E4002B">${esc(l2)}</text>`:"");
    case "exchangeTag": return `<rect x="-180" y="-50" width="360" height="100" rx="10" fill="#0ca678" stroke="#fff" stroke-width="3"/>` + T("Mukta,sans-serif",28,-6,"#fff") + (l2?`<text x="0" y="34" text-anchor="middle" font-family="${F}" font-size="40" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:"");
    case "loanFull":    return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#7b1fa2" stroke="#fff" stroke-width="3"/>` + T("Mukta,sans-serif",28,-8,"#fff") + (l2?`<text x="0" y="34" text-anchor="middle" font-family="${F}" font-size="46" font-weight="900" fill="#ffd400">${esc(l2)}</text>`:"");
    case "freeGiftTag": return `<rect x="-175" y="-56" width="350" height="112" rx="12" fill="#E4002B" stroke="#ffd400" stroke-width="4"/>` + T("Mukta,sans-serif",28,-8,"#ffd400") + (l2?`<text x="0" y="34" text-anchor="middle" font-family="${F}" font-size="38" font-weight="900" fill="#fff">${esc(l2)}</text>`:"");
    default: return null;
  }
};
const _origBuildBadge = buildBadge.bind({});
...
const AI_BG_PRESETS = [
  ...
  ["glossy red showroom floor with spotlights, luxury background, empty centre", "🔴 रेड शोरूम"],
  // ── सड़क / लोकेशन ──
  ...
  ["Bhopal city skyline at sunset, modern urban, empty centre", "🌇 भोपाल शहर"],
  // ── त्यौहार ──
  ...
  ["Raksha Bandhan soft pink floral background, silk thread, rakhi, empty centre", "💝 रक्षाबंधन"],
  ["independence day patriotic tricolor background, soft, empty centre", "🇮🇳 तिरंगा (देशभक्ति)"],
  ["Republic Day parade background, tricolor confetti, national flag, empty centre", "🎖️ गणतंत्र दिवस"],
  ["Christmas winter background, snowflakes, bokeh lights, empty centre", "🎄 क्रिसमस"],
  ["New Year celebration background, fireworks, golden confetti, empty centre", "🎆 नया साल"],
  ["Eid Mubarak crescent moon stars, soft gradient, festive, empty centre", "🌙 ईद"],
  // ── रंग / एब्स्ट्रैक्ट ──
  ...
  ["golden yellow gradient background, luxury feel, empty centre", "🟡 गोल्डन"],
  ["pure white minimal background, soft shadow, professional, empty centre", "⬜ सफ़ेद (clean)"],
  ["dark black dramatic background, neon glow edges, empty centre", "⬛ काला (dramatic)"],
];
const PICK = [
  { group: "⭐ इमोजी", items: [
    ["emoji:star", "⭐ स्टार"], ["emoji:heart", "❤️ दिल"], ["emoji:flame", "🔥 आग"],
    ["emoji:gift", "🎁 गिफ्ट"], ["emoji:sparkle", "✨ चमक"], ["emoji:party", "🎉 पार्टी"],
    ["emoji:thumbsup", "👍 लाइक"], ["emoji:crown", "👑 ताज"], ["emoji:check", "✅ चेक"],
    ["emoji:sealOffer", "🔴 ऑफर सील"], ["emoji:sealSale", "🟡 सेल सील"], ["emoji:badgeNew", "🔵 NEW"],
  ]},
  { group: "📍 सिंबल / आइकॉन", items: [
    ["emoji:loc", "📍 लोकेशन"], ["emoji:phone", "📞 फ़ोन"], ["emoji:whatsapp", "💬 WhatsApp"],
    ["emoji:tick", "✔️ टिक"], ["emoji:rupee", "₹ रुपया"], ["emoji:arrowR", "➡️ तीर"],
    ["emoji:arrowD", "⬇️ नीचे"],
  ]},
  { group: "🏷️ ऑफर बैज (text बदलें)", items: BADGES.map((b) => ["badge:" + b.style, b.label]) },
  { group: "📢 CTA बटन", items: [
    ["cta:आज ही बुक करें", "👉 आज ही बुक करें"],
    ["cta:अभी कॉल करें", "📞 अभी कॉल करें"],
    ["cta:शोरूम विज़िट करें", "🏬 शोरूम विज़िट करें"],
    ["cta:लिमिटेड ऑफर", "🔥 लिमिटेड ऑफर"],
    ["cta:बेस्ट डील", "🏆 बेस्ट डील"],
    ["cta:Free Test Ride", "🏍️ Free Test Ride"],
    ["cta:EMI से लें", "💳 EMI से लें"],
    ["cta:Exchange करें", "🔄 Exchange करें"],
    ["cta:Offer Valid Till", "⏰ Offer Valid Till"],
    ["cta:T&C Apply*", "📋 T&C Apply*"],
  ]},
];
...
export default function PromoEditor({ apiBase, token, brandId, onSent }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(apiBase.replace(/\/+$/, "") + getBrand(brandId).logo, { mode: "cors" });
  ...
  // ⚠️ brand-aware defaults — पहले सब कुछ VP Honda/Shine पर fix था
  const B0 = getBrand(brandId);
  const [f, setF] = useState({
    model: B0.products[0], price: "", down: "", cashback: "",
    features: B0.features.slice(0, 4).join(", "),
    phone: B0.phone, place: B0.address, brand: B0.name,
    tagline: B0.kind === "ice2w" ? "SOLID माइलेज़" : B0.kind === "ev2w" ? "दमदार रेंज" : "मज़बूत लोड क्षमता",
  });
  const [touched, setTouched] = useState(false);
  useEffect(() => {
    if (touched) return;
    const b = getBrand(brandId);
    setF((x) => ({ ...x, model: b.products[0], features: b.features.slice(0, 4).join(", "),
      phone: b.phone, place: b.address, brand: b.name,
      tagline: b.kind === "ice2w" ? "SOLID माइलेज़" : b.kind === "ev2w" ? "दमदार रेंज" : "मज़बूत लोड क्षमता" }));
  }, [brandId, touched]);
  ...
  function addSticker(name) {
    const id = "s" + Date.now();
    // हर sticker अलग random position पर — overlap नहीं होगा
    const x = 200 + Math.floor(Math.random() * 500);
    const y = 150 + Math.floor(Math.random() * 500);
    setStickers((a) => [...a, { id, kind: "shape", name, x, y, r: 60 }]);
    setSelStk(id); setNote("sticker जुड़ा — drag करें");
  }
  function addBadge(style, def) {
    const id = "b" + Date.now();
    // हर badge अलग position — 4 corners में rotate
    const positions = [{x:120,y:140},{x:820,y:140},{x:120,y:800},{x:820,y:800},{x:540,y:100},{x:100,y:500}];
    setStickers((a) => {
      const idx = a.filter(s=>s.kind==="badge").length % positions.length;
      const {x, y} = positions[idx];
      return [...a, { id, kind: "badge", style, text: def, x, y, r: 80 }];
    });
    setSelStk(id); setNote("बैज जुड़ा — drag करें, नीचे text बदलें");
  }
  ...
  async function aiBackground() {
    ...
    try {
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/ai-bg", { method: "POST", headers: { "Content-Type": "application/json", ...(token ? { Authorization: "Bearer " + token } : {}) }, body: JSON.stringify({ brand: brandId || "vp_honda", prompt: (bgPrompt || ((f.model || "") + " premium showroom backdrop")), w: 1080, h: 1080 }) });
  ...
  async function aiText() {
    ...
    try {
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/ai-text", { method: "POST", headers: { "Content-Type": "application/json", ...(token ? { Authorization: "Bearer " + token } : {}) }, body: JSON.stringify({ brand: brandId || "vp_honda", type: "vigyapan" }) });
      const j = await res.json();
  ...
  function download(story) {
    renderCanvas(story).then((c) => { const a = document.createElement("a"); a.href = c.toDataURL("image/png"); a.download = brandId + "-" + (story ? "story" : "square") + ".png"; a.click(); })
  ...
  async function sendToQueue() {
    ...
    try {
      ...
      const res = await fetch(apiBase.replace(/\/+$/, "") + "/api/promo-image", { method: "POST", headers: { Authorization: "Bearer " + token }, body: fd });
  ...
  return (
    <div className="rounded-2xl bg-neutral-900 border border-neutral-800 p-3 mb-5 space-y-3">
      ...
      <button type="button" onClick={() => { vib(60); sendToQueue(); }} className="w-full rounded-xl py-3 font-semibold text-black" style={{ background: GOLD }}>📤 Review में भेजें (फिर FB/IG/WhatsApp post करें)</button>
      <div className="grid grid-cols-2 gap-2">
