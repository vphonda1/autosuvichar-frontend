...
export default function DeliveryEditor({ apiBase, token, brandId, onSent, draft }) {
  ...
  //     होगी। यह सिर्फ़ नाम/गाड़ी/दुकान का data भर देता है, फिर आप photo
  //     चुनिए। command-router.js भेजता है customerName/bikeName/text —
  //     यहाँ की सूची f.name/f.vehicle/f.sub से मिलाया गया है।
  useEffect(() => {
    if (!draft) return;
    setTouched(true);
    setF((x) => ({
      ...x,
      name: draft.customerName || x.name,
      vehicle: draft.bikeName || x.vehicle,
      sub: draft.text || x.sub,
      phone: draft.phone || x.phone,
      place: draft.shopSub || x.place,
      brand: draft.shopName || x.brand,
    }));
  }, [draft]);
